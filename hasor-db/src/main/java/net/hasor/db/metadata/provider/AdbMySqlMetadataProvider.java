    ...
    protected AdbMySqlColumn convertColumn(Map<String, Object> recordMap, List<String> primaryKeyColumnList) {
        ...
        if (StringUtils.isNotBlank(extra)) {
            if (extra.toLowerCase().contains("on update current_timestamp")) {
                column.setOnCurrentUpdateType(AdbMySqlOnCurrentUpdateType.CurrentTimestamp);
            }
            if (extra.toLowerCase().contains("on update current_date")) {
                column.setOnCurrentUpdateType(AdbMySqlOnCurrentUpdateType.CurrentDate);
            }
            if (extra.toLowerCase().contains("on update current_time")) {
                column.setOnCurrentUpdateType(AdbMySqlOnCurrentUpdateType.CurrentTime);
            }
        }
