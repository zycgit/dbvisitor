...
description: Mapper API organizes access to different data sources with Java interfaces. SQL/DSL can come from method annotations, BaseMapper common CRUD, or Mapper files.
---
...
Mappers can organize relational SQL or execute NoSQL DSL and native commands through the matching driver while preserving interface calls, parameter binding and result mapping patterns. `BaseMapper` operations are generated by dialects; check [data source differences](../../features/overview.md) for their supported scope.

## Pick Your Approach

| Your situation | Recommended approach | Notes |
|---|---|---|
| SQL/DSL is short and clearest next to the interface | **Method Annotations** | `@Query` / `@Insert` / `@Update` / `@Delete` to declare commands |
...
| SQL/DSL is long or needs centralized maintenance | **Mapper File** | XML centralizes command management; interface methods reference statements in the file |
