...
<span id="argument-options" />

# 6.6 Argument Options
...
select * from users where age = #{age,javaType=java.lang.Integer}
```
...
{call test_procedure(#{arg, 
            mode=out,
            jdbcType=varchar,
            typeHandler=net.hasor.dbvisitor.types.handler.string.StringAsMonthDayTypeHandler})
}
```

```text title='Example 7: Alias an OUT argument and save the OUT result as outArg'
