...
<span id="statement-generation-rules" />
...
1. **Context-aware connectors**: The rule inspects the preceding `WHERE`, `AND`, or `OR` to decide whether to add a connector. Write the predicate directly, for example `WHERE @{and, x=:x}`; do not add a leading AND inside the rule.
...
- **Context-aware Connectors**: Checks preceding SQL to decide whether to add `AND`; it does not strip a handwritten leading `AND` from the rule body.
- **Preserve Existing Connectors**: If preceding SQL ends with `OR`, no extra `AND` is added.
:::
...
- **`@{set, sql fragment}`**: Appends an assignment and automatically manages commas. Unlike `and`/`or`, `set` **allows argument values to be null**.
...
select * from users where
    ...
    -- For conditional inclusion, replace the preceding rule with:
...
<select id="queryUser">
    ...
    <!-- For conditional inclusion, wrap the preceding include in if; do not include it twice. -->
</select>
```
...
:::caution[Note]
The content must parse into exactly one bound parameter, such as `:loginPassword`, `#{loginPassword}`, or `?`. A bare name is not a bound parameter. MD5 is hashing, not encryption, and should not be used for new password storage; this example only illustrates compatibility with an existing MD5 field.
:::

<Tabs>
  <TabItem value="rule-style" label="Using Rules" default>

**Example**: Hash matching for an existing MD5 password field.
