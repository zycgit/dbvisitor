...
- `userName` is the input argument.
- `outName` is the output argument.
- The SELECT statement produces a separate result set.
...
- `userName` is the input argument.
- `tableCursor` returns the result set through an OUT cursor.
...
When a function value is returned as a query column, use a JdbcTemplate query method. For example, with MySQL:

```java
Integer value = jdbc.queryForObject("SELECT ABS(?)", new Object[] { -5 }, Integer.class);
```

Use list or object queries for row results. JDBC function return parameters instead require a callable statement and an OUT declaration. The accepted function syntax and return shape depend on the data source; see [Programmatic API differences](../../../features/differences/jdbc.md).
