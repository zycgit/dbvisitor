...
sidebar_position: 4
title: Dynamic SQL
description: dbVisitor offers MyBatis-style Dynamic SQL via XML.
---
...
- [&lt;if&gt;](./dynamic_sql#if): conditional fragments.
- [&lt;choose&gt; / &lt;when&gt; / &lt;otherwise&gt;](./dynamic_sql#choose): multi-branch condition selection.
- [&lt;trim&gt; / &lt;where&gt; / &lt;set&gt;](./dynamic_sql#trim): help avoid malformed SQL when generating specific statements.
- [&lt;foreach&gt;](./dynamic_sql#foreach): iterate collections.
- [&lt;bind&gt;](./dynamic_sql#bind): create variables via OGNL expressions and bind them to the context.
...
```sql title='Depending on the value of age, the generated statement will be one of the following:'
select * from users;
...
`<set>` only trims separators; it does not supply missing update columns. Reject the update before calling the mapper if all fields are null.

### trim tag

`<trim>` can replace `<where>` or `<set>` for special cases.

```xml title='Use trim tag to replace where tag'
<trim prefix="where" prefixOverrides="and | or">
    ...
</trim>
```

```xml title='Use trim tag to replace set tag'
