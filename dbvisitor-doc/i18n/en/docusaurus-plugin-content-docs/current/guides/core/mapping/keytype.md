...
| Sequence | Fetch next value from a DB sequence, assign, then insert. See [Dialect support](../../api/differences/about#dialect).<br/>Requires @KeySeq to name the sequence. |
...
| uuid32          | Pre-fill with a 32-char UUID, e.g., `4d68040901d24b70bd10c1c8119001e2`.                  |
| uuid36          | Pre-fill with a 36-char UUID, e.g., `4d680409-01d2-4b70-bd10-c1c8119001e2`.                  |
| Sequence::xxxx  | Fetch next value from sequence `xxxx`, assign, then insert. See [Dialect support](../../api/differences/about#dialect). |
