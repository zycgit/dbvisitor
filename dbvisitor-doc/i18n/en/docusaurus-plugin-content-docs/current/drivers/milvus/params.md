...
| `consistencyLevel` | `Strong` / `Bounded` / `Session` / `Eventually` | SDK / collection default | Query consistency. QueryIterator uses collection-default consistency rather than this override; see [consistency controls](../../features/milvus/jdbc.mdx#execution-controls). |
