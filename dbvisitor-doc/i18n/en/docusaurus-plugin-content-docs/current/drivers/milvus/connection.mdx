...
- **Management permissions**: database creation, user management and role management depend on the deployment option and account permissions. Use an authorized database for ordinary reads and writes; see [database creation API availability](https://docs.zilliz.com/reference/python/python/Database-create_database).
- **Quotas and rate limits**: collection, partition and data operations are subject to Cloud quotas. Reduce request frequency when rate-limited instead of retrying in a tight loop; see [Cloud limits](https://docs.zilliz.com/docs/limits) for current values.
...
With HikariCP, explicitly set `connectionTestQuery`: the current shared driver does not implement `Connection.isValid()`. Use `PING` to probe the server starting with 6.8.1, or `SHOW TABLES` on older versions. `SELECT 1` returns a local constant and does not check server connectivity. Keep auto-commit enabled and do not configure a transaction isolation level. Configure the URL, authentication, and TLS as for a normal JDBC connection, and close the pool when the application exits.
