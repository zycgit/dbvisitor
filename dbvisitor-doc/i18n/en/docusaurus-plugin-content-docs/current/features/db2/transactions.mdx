...
Db2 names differ from JDBC enum names: READ_UNCOMMITTED maps to UR, READ_COMMITTED to CS, REPEATABLE_READ to RS, and SERIALIZABLE to RR. For Db2 RR, select `Isolation.SERIALIZABLE`, not REPEATABLE_READ. `Isolation.DEFAULT` keeps the current connection setting. See [database isolation documentation](https://www.ibm.com/docs/en/db2/12.1?topic=applications-data-server-driver-jdbc-sqlj-isolation-levels).
