...
| Primary Key Generation | `_id` supports `useGeneratedKeys` backfill |
| Pagination | Paginated via Page object (from/size) |
| Batch Writes | executeBatch not supported |
| Stored Procedures | Not supported |
| Vector Search | Elastic 7 (script_score) / Elastic 8 (native kNN) |

**Not supported:** executeBatch, Stored Procedures

## Concept Analogy

Execution results of different ElasticSearch commands fall into three categories:
- **Update count** — analogous to INSERT/UPDATE/DELETE, obtained via `executeUpdate`
- **Single/Multi-row results** — analogous to SELECT result sets, first column is `_ID`, second is `_DOC` (both strings)
