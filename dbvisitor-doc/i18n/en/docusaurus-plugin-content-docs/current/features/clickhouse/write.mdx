...
## Wait Before Reading {#wait-for-writes}
...
## Update and Delete Return Values {#affected-rows}
...
Invalid commands report errors to the caller, but MergeTree sorting and primary keys do not enforce uniqueness. Repeated IDs therefore do not cause duplicate-key errors. Batch calls cannot rely on such errors to detect duplicates; see [Duplicate Records](./conflict.mdx).

## Transaction support {#transaction-propagation}

See [Transaction Support](transactions.md) for transaction APIs, propagation, and isolation limits.
