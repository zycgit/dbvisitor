...
The Milvus adapter uses standard SQL-style syntax, with behavior very close to traditional relational database JDBC:
...
Additionally, Milvus Updates are essentially "Search-to-Upsert" (search then write); full-table Updates on massive data are not recommended.
:::
...
The Builder API provides a type-safe, fluent way to operate Milvus with the same usage as RDBMS. For details, see [Builder API](../../guides/core/lambda/about).
...
:::tip[Tip]
For the [Core API annotations](../../guides/core/mapper/about), all annotations work normally on Milvus except `@Call`.
...
Use `vectorByL2` / `vectorByCosine` / `vectorByIP` to filter records whose distance is less than a threshold — they are WHERE conditions and can be freely combined with other conditions:
