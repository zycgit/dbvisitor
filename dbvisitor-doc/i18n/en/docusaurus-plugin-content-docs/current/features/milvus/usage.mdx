...
This page shows how dbVisitor APIs execute the adapter's SQL subset. See [Install and Use](../../drivers/milvus/usecase.mdx) for dependencies, connections, TLS and a complete JDBC program, and the [support matrix](../../drivers/milvus/compatibility.md) for version requirements and feature boundaries. Examples use two-dimensional FloatVector fields; the application supplies dataSource, connection, entity accessors and Session.

**Supported capabilities:**

- Operate data using SQL-style commands ([supported command list](../../drivers/milvus/commands))
...
**Unsupported:** transactions/savepoints, JDBC executeBatch, stored procedures and updatable ResultSets. INSERT/UPSERT supports multiple VALUES, lazy Iterable/Iterator inputs and [JDBC generated keys](../../drivers/milvus/usecase.mdx#typed-values); these are not JDBC batch.
...
Use JdbcTemplate to execute SQL-style commands to operate Milvus. Make sure the Milvus data source is properly configured first — see [Milvus Driver Usage Guide](../../drivers/milvus/usecase).
...
This is a single filtered search path. Use [native HYBRID syntax](../../drivers/milvus/commands.md#hybrid) for multiple candidate paths and fused ranking.
...
To use SQL-style syntax directly for vector search (e.g., `ORDER BY field <-> vector`, `vector_range()`, etc.), see the [supported command list](../../drivers/milvus/commands#dql).
:::
