...
- Operate data using Redis commands ([supported command list](./commands))
...
Use JdbcTemplate to execute Redis commands directly. Make sure the Redis data source is properly configured first — see [Redis Driver Usage Guide](./jdbc).
...
:::tip[Tip]
Redis data type command references: [String](./commands#string), [Hash](./commands#hash), [List](./commands#list), [Set](./commands#set), [SortedSet](./commands#storeset)
:::
