...
PostgreSQL accepts the four standard isolation levels, but READ_UNCOMMITTED behaves as READ_COMMITTED. Successfully applying either value does not make their behavior different. `Isolation.DEFAULT` keeps the current connection setting. See [database isolation documentation](https://www.postgresql.org/docs/17/sql-set-transaction.html).
