...
```sql
/* SQL template; ids must be non-empty when the combined condition is enabled */
...
2.  **Dynamic Decision**:
    ...
    *   **Case B: Valid parameters exist**. As long as any one of the parameters is not null, the entire expression will be appended to the SQL as a whole.
...
3.  **Security**: Although writing complex expressions is allowed, all variables (`:age`, etc.) are still processed through JDBC PreparedStatement, binding data values. Table names, column names, and `${...}` text substitution are outside that protection and must be controlled by the application.
