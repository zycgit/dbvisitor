...
If `SqlDialect` defines "what" the database is (metadata), then the `SqlCommandBuilder` instance produced by it is responsible for solving "how to do" (constructing queries).
...
3.  **Reorganization and Simplification of Inheritance Hierarchy**:
    The former standalone SqlCommandBuilder implementation became an interface of the same name, with command-building logic moved into abstract base classes. The new hierarchy is as follows:
