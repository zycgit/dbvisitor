...
In the Java ecosystem, the most common ways to operate MongoDB are using the official `mongo-java-driver` or the Spring family's `spring-data-mongodb`. These tools are powerful, but for developers accustomed to Relational Database Management Systems (RDBMS) and the MyBatis development pattern, switching to MongoDB often means adapting to a completely new set of APIs and mindsets.

Especially in a hybrid architecture project, if both MySQL and MongoDB exist, the code style of the data access layer can feel very fragmented: on one side are MyBatis Mapper interfaces and XML, and on the other side are `MongoTemplate` chain calls or Repository interfaces. This difference not only increases learning costs but also makes common functions like "pagination queries" difficult to implement uniformly.

This article will introduce how to use **dbVisitor** to operate MongoDB in a "MyBatis-like" way, achieving architectural unity.
