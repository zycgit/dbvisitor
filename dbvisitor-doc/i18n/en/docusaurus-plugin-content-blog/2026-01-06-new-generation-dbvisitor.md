...
2.  **Middleware's Attitude towards JDBC**:
    ...
    *   **Elasticsearch**: Once tried to provide JDBC support, but with many limitations (does not support nested object complex queries), and its SQL coverage is not equivalent to all Query DSL features.
...
In this process, **Dialect** plays a key translator role. It is responsible for generating **proprietary DSLs** (such as MySQL's SQL, MongoDB's BSON Command, Elasticsearch's JSON DSL) that the target data source can understand based on the calling behavior of the upper-layer unified API (such as `.queryForList()`, `.eq()`).
...
*   **NoSQL Support**:
    ...
    *   **Redis**: Accesses key-value data through commands, JDBC, and Mapper; there is no Lambda table-query dialect.

In dbVisitor's world, developers no longer need to refactor the entire data access layer code to introduce a new middleware. **One API, Access Multiple Databases**, this is not just a slogan.
