...
dbVisitor 提供多种参数传递方式，用于覆盖位置参数、名称参数、对象参数和规则参数等场景。

## 使用指引

- [位置参数](./position)，语句中使用 “?” 标记参数，按顺序传入值；Java 数组下标从 0 开始，JDBC `PreparedStatement` 的参数索引从 1 开始。
- [名称参数](./named)，语句中使用如 `:name`、`&name` 或 `#{...}` 写法，可以将 SQL 中的参数名称化。
- [SQL 文本替换](./inject)，语句中使用 `${...}` 写法，可以对已名称化的参数进行取值，并将结果注入到 SQL 语句中。
