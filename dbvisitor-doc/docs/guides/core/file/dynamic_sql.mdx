...
sidebar_position: 4
title: 动态 SQL
description: dbVisitor 工具提供基于 XML 动态 SQL 配置，并且采用了大家熟悉的 MyBatis 风格。
---
...
- [&lt;if&gt; 标签](#if)，用于判断某个条件满足。
- [&lt;choose&gt;、&lt;when&gt;、&lt;otherwise&gt; 标签](#choose)，多分支条件选择。
- [&lt;trim&gt;、&lt;where&gt;、&lt;set&gt; 标签](#trim)，用于生成结构正确的条件或 SET 片段。
- [&lt;foreach&gt; 标签](#foreach)，foreach。
- [&lt;bind&gt; 标签](#bind)，通过 ognl 表达式创建变量并绑定到上下文。
...
&lt;trim&gt;、&lt;where&gt;、&lt;set&gt; 三个标签用于生成结构正确的 SQL 片段。以下示例展示未使用这些标签时容易出现的问题：
...
`<set>` 只清理首尾分隔符，不会自动补充更新列。所有字段都为空时，应在调用前拒绝这次更新。
