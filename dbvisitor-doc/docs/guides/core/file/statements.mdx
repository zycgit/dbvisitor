...
- [&lt;selectKey&gt; 标签](./statements#selectKey)，在 &lt;insert&gt; 标签内可以使用 &lt;selectKey&gt; 标签执行额外查询并回填主键。
- [&lt;execute&gt; 标签](./statements#execute)，可以用来执行任意的 SQL 语句。
- [&lt;sql&gt; 标签](./statements#sql)，定义 SQL 片段。

## 命令执行 {#execution}

这些标签也可以承载数据源的原生命令，不要求命令必须是 SQL。加载 Mapper 文件后，可通过 `namespace.statementId` 调用其中的语句；用法见[调用文件 Mapper](../mapper/file_statement.md)。

## 执行选项 {#execution-options}

`statementType` 选择执行方式，`timeout` 设置超时，`fetchSize` 提供读取批次提示，`resultSetType` 选择结果集类型。查询默认使用预编译语句；需要滚动结果时，驱动也必须支持相应的 `resultSetType`。

各标签可配置的属性见下文。`fetchSize` 是读取批次提示，不是返回记录的总数限制。

## select 标签 {#select}

&lt;select&gt; 标签等同于 [@Query 注解](../mapper/annotation_query)，用于配置 SELECT 语句，对于简单的情况只需要具体的 SQL 语句即可。例如:
...
| resultType         | <TagGray/> 将返回的预期类型的完全限定类名或 [类型别名](./type_alias)。提示：在集合的情况下，这应该是集合的元素类型，而不是集合本身的类型。<br/>- 可以使用 `resultMap` 和 `resultType` 其中一个属性但不应同时使用它们。如果都没有配置将会参照映射的接口返回值类型决定。                                                                                                                                                                                                                                                                                                                                                     |
...
| bindOut            | <TagGray/> 当 Query 中使用了 [多值](../jdbc/multiple_results) 或者 [存储过程调用](../jdbc/procedure)，使用 bindOut 参数可以绑定其中的输出参数。<br/>请注意：一旦使用了该参数那么接口返回值类型必须为 **Map&lt;String,Object&gt;** 类型。                                                                                                                                                                                                                                                                                                                                                         |

### 存储过程调用 {#callable}

调用存储过程时设置 `statementType="CALLABLE"`，通过参数中的 `mode` 声明 IN、OUT 或 INOUT，使用 `bindOut` 选择输出。输出参数及游标的写法见[存储过程调用](../jdbc/procedure)。数据库和 JDBC 驱动需要提供对应能力。

## update 和 delete 标签 {#update_delete}

&lt;update&gt; 标签等同于 [@Update 注解](../mapper/annotation_update)、&lt;delete&gt; 标签等同于 [@Delete 注解](../mapper/annotation_delete)。用于配置对应的 UPDATE、DELETE 语句。
...
&lt;insert&gt; 标签等同于 [@Insert 注解](../mapper/annotation_insert)，用于配置对应的 INSERT 语句。
...
- 手工赋值：在 INSERT 中直接绑定调用方提供的主键。
- 生成键回填：使用 `useGeneratedKeys`、`keyProperty`、`keyColumn` 接收数据库返回的生成键。
- 查询后回填：使用 [selectKey 标签](#selectKey)，在 INSERT 前或后执行查询，将结果写回参数属性。

`<insert>` 可以通过 `useGeneratedKeys`、`keyProperty`、`keyColumn` 声明主键回填。不同数据库的主键返回方式差异较大，具体用法请阅读 [数据源特性](../../../features/overview)。Generated keys 的通用说明见 [Generated Keys](../mapper/annotation_insert#generated-keys)。

## selectKey 标签 {#selectKey}

&lt;selectKey&gt; 标签需要和 &lt;insert&gt; 标签一起才能工作，它等同于 [@SelectKeySql 注解](../mapper/annotation_insert#selectkey)。

新增数据后的后续逻辑经常需要使用新记录的主键。
可以使用 &lt;selectKey&gt; 标签执行额外的主键查询，由框架回填参数对象，无需业务代码手动查询和赋值。
另外有些业务需要自定义数据表的主键，这个时候也可以使用 &lt;selectKey&gt; 标签来实现，它可以随意的设置生成主键的方式。

以下 MySQL 示例使用字符串 UUID，要求 `id` 为可容纳 36 个字符的字符串列和 Java 字符串属性。
...
&lt;execute&gt; 标签等同于 [@Execute 注解](../mapper/annotation_execute)。用于配置任意 SQL 语句。
