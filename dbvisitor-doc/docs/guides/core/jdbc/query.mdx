...
JdbcTemplate 提供多种读取结果集的 API；方法名称并不会把连接或数据库操作强制设为只读。大多数接口接受 SQL 字符串和参数（对象或数组），返回由结果集构建的对象；部分接口支持回调/映射器以自定义处理过程。
...
返回映射到指定类型的单行。常见场景：聚合查询、主键查找。无结果时返回 null，多行时抛出异常；接收可能为空的标量时使用包装类型。
...
回调避免应用收集完整结果集，但是否按需从服务端读取，还取决于 JDBC 驱动、fetchSize 和连接配置。
...
- [位置参数](../../args/position)（多个值放入数组或集合，方法接收的是 `Object args`，不是可变参数）
...
- 动态 SQL
    ...
    - 使用 `${...}` 进行 [SQL 文本替换](../../args/inject) 时虽然不会参与 PreparedStatement 但也算所传参的一种。
