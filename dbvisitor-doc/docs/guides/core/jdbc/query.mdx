...
查询值区别于查询对象是在于值是可映射的最小类型单元，可以是 [TypeHandlerRegistry](../../types/handlers/about) 注册器中的类型。如：`Stirng`、`Int`、`Date` 等。查询值通常有两种情况：
...
- [RowMapper](../../result/row_callback)，使用 RowMapper 的好处是无需关心 ResultSet 的处理过程，只需要将编程工作专注在每一行数据的处理上。
