...
sidebar_position: 9
title: 选项和特性
description: JdbcTemplate 的高级特性和相关参数。
---
import TOCInline from '@theme/TOCInline';

# 选项和特性

JdbcTemplate 提供以下高级特性和相关参数：
...
通过设置 _**ignoreWarnings**_ 为 `false`，dbVisitor 将在 SQL 警告时抛出异常。
...
它不保证数据库只扫描或传输这些行。需要限制查询规模时，应结合数据库支持的 SQL 分页或 LIMIT，并查看所用驱动的说明。
:::
