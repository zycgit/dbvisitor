...
- 默认情况下在多值查询中，所有返回的结果集会统一采用 **[List/Map](../../result/for_map)** 结构。
  即：每个结果集对应一个 `List<Map<String, Object>>`，与更新数一起存入返回的 `Map<String, Object>`。未指定名称时，键为 `#result-set-N` 或 `#update-count-N`，N 是从 1 开始的结果序号。
...
该示例中：
- 第一条语句使用 resultUpdate 规则，规则为更新数设定了名称 `upd`。
- 第二条语句使用 resultSet 规则，规则为结果集设定了名称 `res1` 和类型 `net.demo.dto.User`。
- 第三条语句使用 resultSet 规则，规则为结果集设定了名称 `res2` 和类型 `net.demo.dto.User`。

```java title="2. 执行含有 ‘@{resultSet} 规则’ 的查询并获取结果集"
...
List<User> result1 = (List<User>) result.get("res1"); // 对应 语句的结果
List<User> result2 = (List<User>) result.get("res2"); // 对应 语句的结果
```
