...
| Sequence | 先从数据库序列中获取最新值，填充到属性后再执行数据库插入。需要所选方言实现 `SeqSqlDialect`；Oracle、SQL Server 内置方言不能直接使用此方式，可改用 `selectKey`。兼容性请参考 [数据库支持性](../../../features/support#dialect)。<br/>- 使用该选项需要同时使用 @KeySeq 注解标识序列的名字。 |
