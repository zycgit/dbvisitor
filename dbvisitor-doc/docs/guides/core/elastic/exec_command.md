...
title: 执行命令
description: 使用 JdbcTemplate 执行原始的 ElasticSearch 命令并进行读写数据。
---

使用 JdbcTemplate 可以直接执行 ElasticSearch 命令并进行读写数据操作，在此之前请确保已经正确配置好 ElasticSearch 数据源，具体请参考 [ElasticSearch 驱动使用指南](../../drivers/elastic/usecase)。
