...
title: 构造器 API
description: 介绍如何使用 LambdaTemplate 进行类型安全的 ElasticSearch 操作。
---

# 构造器 API (LambdaTemplate)

构造器 API（也称为 Lambda API）提供了一种类型安全、流式调用的方式来操作数据库。对于 ElasticSearch，`LambdaTemplate` 提供了极大的灵活性，允许你通过 Java Lambda 表达式构建查询和更新条件。
...
Connection conn = ...; // 获取 ElasticSearch 的 JDBC 连接
