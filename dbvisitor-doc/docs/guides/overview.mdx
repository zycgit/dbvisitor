...
- dbvisitor-adapter，用于 JDBC 驱动程序适配器的模块，用于连接诸如 NoSQL 数据库之类的数据库。
  - 例如：[Redis](../features/redis)、[MongoDB](../features/mongo)、[ElasticSearch](../features/elastic)。
...
- dbVisitor 使用统一的 API 操作非关系型数据库 [Redis](../features/redis)、[MongoDB](../features/mongo)、[ElasticSearch](../features/elastic)。
