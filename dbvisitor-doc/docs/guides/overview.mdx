...
- dbvisitor-integration，是 dbVisitor 与主流框架的集成模块。支持
  [Spring](./yourproject/with_spring)、[SpringBoot](./yourproject/with_springboot)、
  [Solon](./yourproject/with_solon)、[Hasor](./yourproject/with_hasor)、[Guice](./yourproject/with_guice) 等。
- dbvisitor-adapter，用于 JDBC 驱动程序适配器的模块，用于连接诸如 NoSQL 数据库之类的数据库。例如：
  [Redis](./core/redis/about)、[MongoDB](./core/mongo/about) 等。
