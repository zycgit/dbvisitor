...
- dbvisitor-adapter，用于 JDBC 驱动程序适配器的模块，用于连接诸如 NoSQL 数据库之类的数据库。
  - 例如：[Redis](./core/redis/about)、[MongoDB](./core/mongo/about)、[ElasticSearch](./core/elastic/about)。
...
- dbVisitor 使用统一的 API 操作非关系型数据库 [Redis](./core/redis/about)、[MongoDB](./core/mongo/about)、[ElasticSearch](./core/elastic/about)。
