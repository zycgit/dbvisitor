...
- dbvisitor-adapter，用于 JDBC 驱动程序适配器的模块，用于连接诸如 NoSQL 数据库之类的数据库。
  - 例如：[Redis](../features/redis/about.md)、[MongoDB](../features/mongo/about.md)、[ElasticSearch](../features/elastic/about.md)。
...
- 已发布模块可通过 [Maven Central](https://central.sonatype.com/search?q=dbvisitor) 获取；SNAPSHOT 版本需使用相应快照仓库或从源码本地安装。

## JVM 兼容性

dbVisitor 6.7.1 起要求 Java 17 或更高版本；6.7.0 及更早版本的运行要求请参阅对应版本文档。所有模块和第三方 JDBC 驱动还需满足各自的依赖要求。

## 入门

- dbVisitor 支持通过原始 SQL 方式利用 [编程式 API](./api/jdbc) 来访问数据库。
- dbVisitor 支持在接口上通过注解标记以 [声明式 API](./api/mapper) 方式来访问数据库。
- dbVisitor 支持通过 [构造器](./api/lambda) 方式用同一种 API 来访问所有框架支持的数据库。
...
- dbVisitor 使用统一的 API 操作非关系型数据库 [Redis](../features/redis/about.md)、[MongoDB](../features/mongo/about.md)、[ElasticSearch](../features/elastic/about.md)。
...
- 最新正式版：v{Vars.lastReleaseVer} ({Vars.lastReleaseTime})
- 当前使用指南对应：{Vars.docsVersion}；开发版需要从源码构建，新增能力不要直接套用到旧版。
