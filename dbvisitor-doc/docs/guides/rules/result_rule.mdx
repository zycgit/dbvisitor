...
结果规则用于对查询结果进行后置处理，也可用于存储过程等复杂场景下的结果辅助处理。
...
- [多结果集查询](../core/jdbc/multiple_results)

**属性说明**：

| 名称 | 类型 | 说明 |
| :--- | :--- | :--- |
| `name` | String | 该结果集在返回 Map 中的名称。 |
...
`@{defaultResult}` 相当于 `@{resultSet}` 的一次执行范围内默认配置。当一个查询返回多个结果集时，可以使用此规则避免为每个结果集重复配置。
