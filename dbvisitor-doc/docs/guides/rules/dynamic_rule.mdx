...
|------|-------------------|-------------------------------|
| 生效判断 | SQL 片段中的至少一个参数**非 null** 时生效（文本注入另见下文） | **OGNL 条件表达式**为 `true` 时生效 |
| 空值参数 | 参数全部为 null 或没有参数时，整个片段被**丢弃** | 条件满足时参数为 null 也会**保留** |
| 典型用途 | 简洁写法，自动判空 | 需要自定义判断逻辑 |

### 特性说明

1. **上下文连接符**：规则检查前文末尾的 `WHERE`、`AND`、`OR` 等，决定是否补连接符；规则内容应直接写条件，例如 `WHERE @{and, x=:x}`。不要在规则内部再写前导 AND。
2. **智能空值丢弃**：
...
:::info 智能特性
与 AND 规则一致，根据前文补 `WHERE` 或 `OR`；规则内部应直接写条件，不要再写前导连接符。
:::
...
:::caution 注意事项
本规则**不会**自动补全 `AND`/`OR` 前缀，需在规则内手动写明（如示例中的 `and id in ...`）。集合为空或 null 时规则不输出内容；若空集合应表示“不匹配任何行”，应在调用前返回空结果或显式追加 `1=0`，不要让条件被省略后扩大查询或修改范围。
:::
...
<select id="queryUser">
    ...
    <!-- 条件引用时，可用 if 包裹上面的 include；不要重复引用同一条件。 -->
</select>
```
...
:::caution[注意]
规则内容必须解析出且仅解析出一个绑定参数，如 `:loginPassword`、`#{loginPassword}` 或 `?`；裸名称不是绑定参数。MD5 是哈希而非加密，不应用于新的密码存储方案；下面仅演示兼容既有 MD5 字段。
:::
