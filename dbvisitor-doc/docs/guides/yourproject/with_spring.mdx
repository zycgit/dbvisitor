...
Spring 是一个广泛使用的 Java 应用框架，提供 IoC 容器、事务管理和 Spring Boot 自动配置等能力。
...
| dbvisitor.mapper-locations          | <TagGray/> 要扫描加载的 Mapper 映射文件所在路径，默认值为 `classpath*:/dbvisitor/mapper/**/*.xml`，如果有多个可以通过以下任意一个字符进行分割 `,; \t\n` |
