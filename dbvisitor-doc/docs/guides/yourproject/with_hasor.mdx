...
Hasor 是一个轻量级 Java 应用框架，提供 IoC、AOP、Web 开发和模块化扩展能力。
...
首先引入依赖包，当前指南版本：<Highlight color="rgb(227 17 108)">{Vars.docsVersion}</Highlight>
...
```java title='启动 Hasor'
// 根据配置创建数据源、装配 dbVisitor，并注册 Mapper
...
- TransactionManager 接口，是开发 [编程式](../transaction/program) 事务的 API 接口
- TransactionTemplate 接口，是开发 [模版](../transaction/template) 事务的 API 接口
- Configuration 配置类
- Session 类
- 自定义 Mapper
...
| dbvisitor.dialect                   | <TagGray/> 默认使用的 **[数据库方言](../../features/differences/builder)** |
