...
- **管理权限**：建库、用户及角色管理等操作取决于产品形态和账号权限。普通读写使用已授权的数据库；参见[建库 API 的适用范围](https://docs.zilliz.com/reference/python/python/Database-create_database)。
- **配额与限流**：集合、分区及数据操作受云端配额约束。收到限流错误时降低请求频率，不要立即循环重试；具体数值见[Cloud 限制](https://docs.zilliz.com/docs/limits)。
...
使用 HikariCP 时，需显式设置 `connectionTestQuery`，因为当前公共驱动未实现 `Connection.isValid()`。6.8.1 起使用 `PING` 探测服务端，旧版使用 `SHOW TABLES`。`SELECT 1` 只在本地返回常量，不用于检测服务端连接。保持自动提交模式，不配置事务隔离级别。连接 URL、认证和 TLS 仍按普通 JDBC 连接配置，应用结束时关闭连接池。
