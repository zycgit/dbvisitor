...
description: ElasticSearch 数据源各 API 方式的详细用法。
---
...
**支持的能力：**
- 使用 ElasticSearch DSL 命令操作数据（[支持的命令列表](./commands)）
...
- **单行/多行结果** — 类比 SELECT 结果集，搜索命中通常包含 `_ID` 和 `_DOC`（字符串），并展开 `_source` 字段供对象映射；管理、聚合等命令的列结构见驱动命令手册

---

## 命令方式（JdbcTemplate）{#exec-command}

使用 JdbcTemplate 可以直接执行 ElasticSearch 命令，在此之前请确保已经正确配置好 ElasticSearch 数据源，具体请参考 [ElasticSearch 驱动使用指南](./jdbc)。
...
```java title='查询数据'
// 查询首批匹配结果；未指定 size 时采用 Elasticsearch 默认条数
...
搜索的可见性受 refresh 影响。需要写入后立即搜索时，在写入请求上使用 `?refresh=wait_for`。用于 `term` 精确匹配的字符串字段应预先映射为 `keyword`。

## 自动生成 ID {#id-fill}

原生 JDBC 使用 `prepareStatement(command, Statement.RETURN_GENERATED_KEYS)`，执行后调用 `getGeneratedKeys()` 读取 `_id`。Mapper 使用如下配置回填 `info.id`，不要设置 `keyColumn`：
...
其中 `UserWithId` 提供可写的 `String id` 属性。此处说明的是 JDBC/Mapper 生成键；Lambda 的 `KeyType.Auto` 使用列名形式的 JDBC 重载，当前适配器不支持该方式，应自行赋业务主键。

## 构造器 API（LambdaTemplate）{#exec-lambda}

构造器 API 提供了一种类型安全、流式调用的方式操作 ElasticSearch，用法与 RDBMS 一致，详细请参考 [构造器 API](../../guides/core/lambda/about)。
...
:::tip[提示]
对于 [核心 API 提供的注解](../../guides/core/mapper/about) 方式除了 `@Call` 注解不支持之外，其它所有注解都可以在 ElasticSearch 数据源上正常使用。
:::
