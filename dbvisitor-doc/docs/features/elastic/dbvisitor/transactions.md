...
JDBC Elastic 没有提供数据库事务提交与回滚。框架可以组织请求调用，但不能把多次文档写入变成一个可回滚的事务。

例如第一篇文档写入成功、第二篇失败后，第一篇不会自动删除或恢复。
...
当前 JDBC 路径不支持设置事务隔离级别。refresh 控制文档对搜索的可见性，不等于 READ_COMMITTED 或 REPEATABLE_READ 等事务隔离。
