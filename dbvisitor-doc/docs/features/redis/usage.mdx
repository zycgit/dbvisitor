...
title: dbVisitor API 用法
description: Redis 数据源各 API 方式的详细用法。
---
...
**支持的能力：**
- 使用 Redis 命令操作数据（[支持的命令列表](./commands)）
- [JdbcTemplate](#exec-command)、[方法注解](#exec-annotation)、[Mapper 文件](#exec-file) 三种使用方式
- [规则](../../guides/rules/about)、[参数传递](../../guides/args/about)、[ResultSetExtractor/RowMapper](../../guides/result/about) 等通用能力
- Mapper File 中的[动态 SQL](../../guides/core/file/dynamic_sql) 标签

**不支持：** 构造器 API、BaseMapper、JDBC Batch、事务、存储过程。结果列仍可通过 RowMapper 映射；JSON 字符串可通过 TypeHandler 还原为对象。
...
使用 JdbcTemplate 可以直接执行 Redis 命令，在此之前请确保已经正确配置好 Redis 数据源，具体请参考 [Redis 驱动使用指南](./jdbc)。
...
:::tip[提示]
对于 [核心 API 提供的注解](../../guides/core/mapper/about) 方式除了 `@Call` 注解不支持之外，其它所有注解都可以在 Redis 数据源上正常使用。
:::
...
:::tip[提示]
Redis 主要数据类型命令参考：[String](./commands#string)、[Hash](./commands#hash)、[List](./commands#list)、[Set](./commands#set)、[StoreSet](./commands#storeset)
:::
...
String incremented = jdbc.queryForString("INCR ?", "myKey"); // 自增后的值为 1
String decremented = jdbc.queryForString("DECR ?", "myKey"); // 自减后的值为 0
```
...
Long length = jdbc.queryForLong("APPEND ? ?", new Object[] { "myKey", "123" }); // 新长度为 10
String prefix = jdbc.queryForString("GETRANGE ? ? ?", new Object[] { "myKey", 0, 5 }); // 包含下标 5，结果为 myValu
```
...
```
- 参考：[HSET 命令](https://redis.io/docs/latest/commands/hset/)、[PAIRS 规则](../../guides/rules/dynamic_rule#pairs)
...
```
- 参考：[LPUSH 命令](https://redis.io/docs/latest/commands/lpush/)、[PAIRS 规则](../../guides/rules/dynamic_rule#pairs)
...
```
- 参考：[SADD 命令](https://redis.io/docs/latest/commands/sadd/)、[PAIRS 规则](../../guides/rules/dynamic_rule#pairs)
...
```
- 参考：[ZADD 命令](https://redis.io/docs/latest/commands/zadd/)、[PAIRS 规则](../../guides/rules/dynamic_rule#pairs)
...
- 除了元组类型，还可以使用 Map 类型、用户自定义类型来存储元素和分数
  - `:v.arg1` 使用了 [名称参数](../../guides/args/named) 方式来传递参数
