...
本页介绍 dbVisitor API 如何执行适配器支持的 SQL。依赖与连接见[安装与连接](../../drivers/milvus/connection.mdx)，TLS 见[连接参数](../../drivers/milvus/params.md)，完整程序见[JDBC 操作用法](./jdbc.mdx)。版本要求与功能边界见[支持范围](./compatibility.md)。下面示例使用二维 FloatVector；片段中的 dataSource、connection、实体访问器和 Session 由应用提供。

**支持的能力：**

- 使用 SQL 风格命令操作数据（[支持的命令列表](./commands)）
...
**不支持：** 事务/savepoint、JDBC executeBatch、存储过程、可更新 ResultSet。INSERT/UPSERT 支持多行 VALUES、惰性 Iterable/Iterator 和 [JDBC 主键回传](./jdbc.mdx#typed-values)；这些不是 JDBC batch。
...
使用 JdbcTemplate 可以直接执行 SQL 风格命令操作 Milvus，在此之前请确保已经正确配置好 Milvus 数据源，具体请参考 [Milvus 驱动使用指南](./jdbc)。
...
这是单路过滤搜索；需要多路候选及融合排名时使用[原生 HYBRID 语法](./commands.md#hybrid)。
...
查询度量必须与已建索引的 metric_type 一致。上文创建的是 L2 索引；尝试 COSINE/IP 示例前，应另建采用对应度量的集合与索引，不要直接在同一个 L2 索引上运行。

使用 `orderByL2` / `orderByCosine` / `orderByIP` 按向量相似度排序，配合 `initPage` 取前 K 条最相似记录：
...
:::tip[SQL 命令方式]
如需直接使用 SQL 风格语法进行向量搜索（如 `ORDER BY field <-> vector`、`vector_range()` 等），请参考 [支持的命令列表](./commands#dql)。
:::
