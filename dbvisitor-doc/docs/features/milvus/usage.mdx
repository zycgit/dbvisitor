...
本页介绍 dbVisitor API 如何执行适配器支持的 SQL；依赖、连接、TLS 和完整 JDBC 程序统一见[安装与使用](../../drivers/milvus/usecase.mdx)。版本要求与功能边界见[支持矩阵](../../drivers/milvus/compatibility.md)。下面示例使用二维 FloatVector；片段中的 dataSource、connection、实体访问器和 Session 由应用提供。
