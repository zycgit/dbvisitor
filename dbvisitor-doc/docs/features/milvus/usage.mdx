...
构造器 API 提供了一种类型安全、流式调用的方式操作 Milvus，用法与 RDBMS 一致，详细请参考 [构造器 API](../../guides/core/lambda/about)。
...
:::tip[提示]
对于 [核心 API 提供的注解](../../guides/core/mapper/about) 方式，除了 `@Call` 注解不支持之外，其它所有注解都可以在 Milvus 数据源上正常使用。
