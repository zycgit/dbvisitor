...
- 重构
  ...
  - **类层级简化**：移除了独立的 SQL/Mongo 构建器实现及胶水类 `MongoBuilderDialect`，构建逻辑合并到方言中；`SqlCommandBuilder` 接口仍保留。
  - **继承体系重组**：公共逻辑下沉至 `AbstractSqlDialect`，形成了 `AbstractBuilderDialect` -> `AbstractSqlDialect` / `MongoDialect` 的清晰层级。
