...
public class JdbcSupportedBehaviorTest {
    ...
    private JdbcStatement statementWith(AdapterCursor cursor, AdapterCursor keys) throws Exception {
        ...
        if (cursor != null) s.container.responseResult(req, cursor, keys);
        else s.container.responseUpdateCount(req, 1, keys);
    ...
    private AdapterMemoryCursor cursor(Object... values) {
        ...
        for (int i = 0; i < values.length; i++) rows[i] = new Object[]{values[i]};
        return new AdapterMemoryCursor(Collections.singletonList(new JdbcColumn("id", "int", "", "", "")), rows);
    }
    ...
    public void preparedBindingsSurviveExecutionUntilCleared() throws Exception {
        try (JdbcPreparedStatement s = (JdbcPreparedStatement) conn.prepareStatement("UPDATE t SET a=?, b=?")) {
            s.setInt(1, 10); s.setString(2, "fixed");
            s.executeUpdate();
    ...
    public void sqlTypeOverloadsAcceptJdbcTypeAndRejectNull() throws Exception {
        try (JdbcPreparedStatement s = (JdbcPreparedStatement) conn.prepareStatement("SELECT ?")) {
            s.setObject(1, 12, JDBCType.INTEGER); s.execute();
            assertEquals(12, s.container.getRequest().getArgMap().get("arg1").getValue());
            assertThrows(SQLException.class, () -> s.setObject(1, 12, (SQLType) null));
        }
        try (CallableStatement s = conn.prepareCall("CALL p")) {
            s.registerOutParameter(1, JDBCType.INTEGER); s.execute();
            assertEquals(42, s.getInt(1));
            assertThrows(SQLException.class, () -> s.registerOutParameter(1, (SQLType) null));
        }
    }
    ...
    public void outValuesAccessibleAndReusable() throws Exception {
        try (CallableStatement s = conn.prepareCall("CALL p")) {
            ...
            s.registerOutParameter(1, Types.INTEGER); s.registerOutParameter(2, Types.VARCHAR);
            s.execute(); assertEquals(42, s.getInt(1)); assertEquals("out_value", s.getString(2));
            assertFalse(s.wasNull());
            s.execute(); assertEquals(42, s.getInt("arg1"));
        }
    }
    ...
    public void statementMetadataWithoutCurrentResultIsNull() throws Exception {
        try (PreparedStatement s = conn.prepareStatement("SELECT id FROM test")) {
            ...
            try (ResultSet r = s.executeQuery()) { assertEquals(2, s.getMetaData().getColumnCount()); }
            assertFalse(s.getMoreResults()); assertNull(s.getMetaData());
        }
        try (PreparedStatement s = conn.prepareStatement("UPDATE t SET x=1")) {
            s.executeUpdate(); assertNull(s.getMetaData());
        }
    }
    ...
    public void differentGetterTypesAndCaseInsensitiveLabels() throws Exception {
        try (Statement s = conn.createStatement(); ResultSet r = s.executeQuery("SELECT id FROM test")) {
            assertTrue(r.next());
            assertEquals(1, r.getInt(1)); assertEquals("1", r.getString("ID")); assertEquals(1L, r.getLong("Id"));
    ...
    public void duplicateLabelsUseFirstColumnWithoutChangingOrdinalTypes() throws Exception {
        List<JdbcColumn> cols = Arrays.asList(new JdbcColumn("id", "int", "", "", ""), new JdbcColumn("ID", "string", "", "", ""));
        try (JdbcStatement s = statementWith(new AdapterMemoryCursor(cols, new Object[][]{{1, "two"}}), null); ResultSet r = s.getResultSet()) {
            r.next(); assertEquals(1, r.getInt("ID")); assertEquals("two", r.getString(2)); assertEquals(1, r.findColumn("id"));
        }
    }
    ...
    public void lookaheadPreservesValuesAndPositions() throws Exception {
        try (JdbcStatement s = statementWith(cursor(1, 2), null); ResultSet r = s.getResultSet()) {
            assertTrue(r.isBeforeFirst()); assertEquals(0, r.getRow());
            assertTrue(r.next()); assertTrue(r.isFirst()); assertFalse(r.isLast()); assertEquals(1, r.getInt(1));
            assertTrue(r.next()); assertTrue(r.isLast()); assertEquals(2, r.getInt(1)); assertEquals(2, r.getRow());
            assertFalse(r.next()); assertEquals(0, r.getRow()); assertTrue(r.isAfterLast()); assertFalse(r.isLast()); assertFalse(r.isFirst());
            assertFalse(r.next()); assertThrows(SQLException.class, () -> r.getInt(1));
        }
    }
    ...
    public void emptyCursorHasNoPosition() throws Exception {
        try (JdbcStatement s = statementWith(cursor(), null); ResultSet r = s.getResultSet()) {
            assertFalse(r.isBeforeFirst()); assertFalse(r.isLast()); assertFalse(r.next());
            assertFalse(r.isBeforeFirst()); assertFalse(r.isAfterLast()); assertEquals(0, r.getRow());
        }
    }
    ...
    public void namedCalendarZoneWorksForOffsetDateTime() throws Exception {
        ...
        List<JdbcColumn> cols = Collections.singletonList(new JdbcColumn("time", AdapterType.OffsetDateTime, "", "", ""));
    ...
    public void closeOnCompletionBeforeAndAfterExecute() throws Exception {
        for (boolean before : new boolean[]{true, false}) {
            ...
            if (before) s.closeOnCompletion();
            ResultSet r = s.executeQuery("SELECT id FROM test");
            if (!before) s.closeOnCompletion();
            assertFalse(s.isClosed()); assertTrue(r.next()); r.close(); assertTrue(s.isClosed());
        }
    }
    ...
    public void closeOnCompletionWaitsForAllKeptResults() throws Exception {
        ...
        s.container.prepareReceive(req); s.container.responseResult(req, cursor(1)); s.container.responseResult(req, cursor(2)); s.container.responseFinish(req);
        s.closeOnCompletion(); ResultSet first = s.getResultSet();
        assertTrue(s.getMoreResults(Statement.KEEP_CURRENT_RESULT));
        ResultSet second = s.getResultSet(); second.close(); assertFalse(s.isClosed());
        first.close(); assertTrue(s.isClosed());
    }

    @Test
    public void reexecutionDoesNotPrematurelyCloseStatement() throws Exception {
        try (Statement s = conn.createStatement()) {
            s.closeOnCompletion(); ResultSet first = s.executeQuery("SELECT id FROM test");
            ResultSet second = s.executeQuery("SELECT id FROM test");
            assertTrue(first.isClosed()); assertFalse(s.isClosed()); second.close(); assertTrue(s.isClosed());
        }
    }
    ...
    public void connectionClosesCurrentAndKeptCursors() throws Exception {
        ...
        JdbcStatement s = statementWith(cursor, keys); ResultSet r = s.getResultSet(); ResultSet k = s.getGeneratedKeys();
        s.getMoreResults(Statement.KEEP_CURRENT_RESULT); conn.close();
        assertTrue(r.isClosed()); assertTrue(k.isClosed()); assertTrue(cursor.isClose()); assertTrue(keys.isClose());
    }

    @Test
    public void generatedKeysUseStableWrapperAndEmptyResult() throws Exception {
        try (JdbcStatement s = statementWith(null, cursor(99))) {
            ResultSet keys = s.getGeneratedKeys(); assertSame(keys, s.getGeneratedKeys());
            assertTrue(keys.next()); assertEquals(99, keys.getInt(1));
            assertFalse(s.getMoreResults()); assertTrue(keys.isClosed());
            ResultSet empty = s.getGeneratedKeys(); assertNotNull(empty); assertFalse(empty.next());
        }
    }
    ...
    public void failedResultCannotBeReadAsSuccessfulUpdateOrGeneratedKeys() throws Exception {
        try (JdbcStatement s = (JdbcStatement)conn.createStatement()) {
            ...
            s.container.prepareReceive(req); s.container.responseFailed(req, new SQLException("failed")); s.container.responseFinish(req);
    ...
    public void arrayContractAndFree() throws Exception {
        ...
        a.free(); a.free(); assertThrows(SQLException.class, a::getArray); assertThrows(SQLException.class, a::getBaseType);
    }
    ...
    public void clobFreeInvalidatesAccessAndPreviouslyOpenedWriter() throws Exception {
        JdbcCob c = new JdbcCob("hello"); java.io.Writer writer = c.setCharacterStream(1);
        c.free(); writer.write("late"); writer.close();
        assertThrows(SQLException.class, c::length); assertThrows(SQLException.class, c::getCharacterStream);
        assertThrows(SQLException.class, () -> c.getSubString(1,1)); assertThrows(SQLException.class, () -> c.setString(1,"x"));
    }
