...
public class SelectWrapperForEntity<T> extends AbstractSelectWrapper<EntityQueryWrapper<T>, T, SFunction<T>> implements EntityQueryWrapper<T> {
    public SelectWrapperForEntity(TableMapping<T> tableMapping, MappingRegistry registry, JdbcTemplate jdbc, SqlDialect dialect) {
        super(tableMapping.entityType(), tableMapping, registry, jdbc, dialect);
    }

    @Override
    public MapQueryWrapper asMap() {
        return new SelectWrapperForMap(this.getTableMapping(), this.registry, this.jdbc, this.dialect);
    }
