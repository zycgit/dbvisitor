 ...
 * @version : 2015年11月10日
 */
public class AnnoPropagationSupportsTest extends AbstractPropagationTest {
    ...
    public void commitTest1() throws Throwable {
        ...
        try (DefaultDs ds = DsUtils.mysqlDs()) {
            ...
            TransactionStatus t1 = txManager.begin(Propagation.NEVER, Isolation.READ_COMMITTED);
            {
                ...
                TransactionHelper.support(this, ds).commitTranTest(jdbc, p -> {
                    assert selectCount(jdbc) == 2; // is SUPPORTS
                });
            }
    ...
    public void commitTest2() throws Throwable {
        ...
        try (DefaultDs ds = DsUtils.mysqlDs()) {
            ...
            assert selectCount(jdbc) == 2; // no tx
        ...
        try (DefaultDs ds = DsUtils.mysqlDs()) {
            ...
            TransactionStatus t1 = txManager.begin(Propagation.REQUIRED, Isolation.READ_COMMITTED);
            {
                try {
                    ...
                    assert selectCount(jdbc) == 2;// rollback t2 do nothing
                }
            }

            txManager.commit(t1);
            assert selectCount(jdbc) == 2;// commit t1
        }
    }
