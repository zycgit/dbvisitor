...
public class CoreCallableStatementExecuteMapperTest {
    ...
    private void reinitPg(Connection con) throws SQLException {
        JdbcTemplate jdbc = new JdbcTemplate(con);
        jdbc.execute("drop table if exists proc_table_for_stat;");
        jdbc.execute("create table proc_table_for_stat( c_id int primary key, c_name varchar(200));");
        jdbc.execute("insert into proc_table_for_stat (c_id,c_name) values (1, 'aaa');");
        ...
        jdbc.execute("DROP PROCEDURE IF EXISTS proc_select_cross_table_for_stat;");
        jdbc.execute(new StringBuilder().append("CREATE OR REPLACE PROCEDURE proc_select_cross_table_for_stat(\n").append("    IN p_name varchar,\n").append("    INOUT p_out varchar,\n").append("    INOUT res1 refcursor,\n").append("    INOUT res2 refcursor\n").append(")\n").append("LANGUAGE plpgsql\n").append("AS $$\n").append("BEGIN\n").append("    p_out := 'outArg';\n").append("    OPEN res1 FOR SELECT * FROM proc_table_for_stat WHERE c_name = p_name;\n").append("    OPEN res2 FOR SELECT * FROM proc_table_for_stat WHERE c_name <> p_name;\n").append("END;\n").append("$$;").toString());
    }

    @Test
    public void executeCall_1() throws Exception {
        Configuration config = new Configuration();
        config.options().setDialect(new PostgreSqlDialect());
        ...
        try (Connection con = DsUtils.pgConn()) {
            ...
            //Assert.assertEquals(result.toString(), "outArg", result.get("arg1"));
            Assert.assertTrue(result.toString(), result.get("res1") instanceof List && ((List) result.get("res1")).size() == 1);
            Assert.assertTrue(result.toString(), result.get("res2") instanceof List && ((List) result.get("res2")).size() == 2);
            con.commit();
        }
    }
    ...
    public void executeCall_2() throws Exception {
        ...
        try (Connection con = DsUtils.pgConn()) {
            ...
            Assert.assertTrue(result.toString(), result.get("res1") instanceof List && ((List) result.get("res1")).size() == 1);
            Assert.assertTrue(result.toString(), result.get("res2") instanceof List && ((List) result.get("res2")).size() == 2);
            Assert.assertEquals(result.toString(), -1, result.get("#update-count-1"));
            con.commit();
        }
    }
