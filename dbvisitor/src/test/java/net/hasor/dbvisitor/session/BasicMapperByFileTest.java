...
public class BasicMapperByFileTest {
    ...
    public void listUserList_1() throws Exception {
        ...
        try (Session s = config.newSession(DsUtils.mysqlConn())) {
            BasicMapperByFile dalExecute = s.createMapper(BasicMapperByFile.class);

            int i = dalExecute.initUser();
            assert i == 2;
            List<UserInfo> empty = dalExecute.listUserList_1("aaa");
            assert empty.isEmpty();

            List<UserInfo> one = dalExecute.listUserList_1("12");
            assert one.size() == 1;

            UserInfo tbUser1 = one.get(0);
    ...
    public void createUser_1() throws Exception {
        ...
        try (Session s = config.newSession(DsUtils.mysqlConn())) {
            ...
            List<UserInfo> one = dalExecute.listUserList_1("112");
            assert one.size() == 1;

            UserInfo tbUser1 = one.get(0);
            assert tbUser1.getUserUuid().equals("111");
            assert tbUser1.getName().equals("112");
            assert tbUser1.getLoginName().equals("113");
            assert tbUser1.getLoginPassword().equals("114");
            assert tbUser1.getEmail().equals("115");
            assert tbUser1.getSeq() == 116;
            assert tbUser1.getRegisterTime() != null;
        }
    }
