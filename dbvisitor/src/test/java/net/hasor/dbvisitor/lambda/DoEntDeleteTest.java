...
public class DoEntDeleteTest {
    ...
    public void delete_1() throws Throwable {
        try (Connection c = DsUtils.h2Conn()) {
            int delete = new LambdaTemplate(c).delete(AnnoUserInfoDTO.class)//
    ...
    public void delete_1_2map() throws Throwable {
        try (Connection c = DsUtils.h2Conn()) {
            int delete = new LambdaTemplate(c).delete(AnnoUserInfoDTO.class).asMap()//
                    .allowEmptyWhere().doDelete();
            assert delete == 3;
        }
    }
    ...
    public void delete_2() throws Throwable {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            List<AnnoUserInfoDTO> tbUsers = new LambdaTemplate(c).query(AnnoUserInfoDTO.class)//
                    .queryForList();
    ...
    public void allowEmptyWhere_1() {
        try (Connection c = DsUtils.h2Conn()) {
            new LambdaTemplate(c).delete(AnnoUserInfoDTO.class)//
                    .doDelete();
            assert false;
        } catch (Exception e) {
    ...
    public void allowEmptyWhere_1_2map() {
        try (Connection c = DsUtils.h2Conn()) {
            new LambdaTemplate(c).delete(AnnoUserInfoDTO.class).asMap()//
                    .doDelete();
            assert false;
        } catch (Exception e) {
            assert e.getMessage().equals("The dangerous DELETE operation, You must call `allowEmptyWhere()` to enable DELETE ALL.");
