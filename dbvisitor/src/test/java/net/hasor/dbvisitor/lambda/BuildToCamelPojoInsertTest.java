...
public class BuildToCamelPojoInsertTest {
    private LambdaTemplate newLambda() throws SQLException {
        Options opt = Options.of().mapUnderscoreToCamelCase(true).dialect(new MySqlDialect());
    ...
    public void insert_1() throws SQLException {
        Insert<UserInfo> lambdaInsert = newLambda().insert(UserInfo.class);
        lambdaInsert.applyMap(mapForData1());

        BoundSql boundSql1 = lambdaInsert.getBoundSql();
        assert boundSql1 instanceof BatchBoundSql;
        assert boundSql1.getSqlString().equals("INSERT INTO user_info (uid, name, login_name, email, seq, create_time) VALUES (?, ?, ?, ?, ?, ?)");
    }
    ...
    public void insert_1_2map() throws SQLException {
        ...
        Insert<Map<String, Object>> lambdaInsert = newLambda().insert(UserInfo.class).asMap();
    ...
    public void insert_duplicateKeyUpdate_1() throws SQLException {
        ...
        BoundSql boundSql1 = lambdaInsert.getBoundSql();
        assert boundSql1 instanceof BatchBoundSql;
        assert boundSql1.getSqlString().equals("INSERT INTO user_info (uid, name, login_name, email, seq, create_time) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE uid=VALUES(uid), name=VALUES(name), login_name=VALUES(login_name), email=VALUES(email), seq=VALUES(seq), create_time=VALUES(create_time)");
    }
    ...
    public void insert_duplicateKeyIgnore_1() throws SQLException {
        ...
        BoundSql boundSql1 = lambdaInsert.getBoundSql();
        assert boundSql1 instanceof BatchBoundSql;
        assert boundSql1.getSqlString().equals("INSERT IGNORE user_info (uid, name, login_name, email, seq, create_time) VALUES (?, ?, ?, ?, ?, ?)");
    }
