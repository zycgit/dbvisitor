...
public class SqlXmlTypeHandlerTest {
    protected void preProc(JdbcTemplate jdbcTemplate) throws SQLException {
        ...
        jdbcTemplate.execute(""                                         //
                + "CREATE OR REPLACE PROCEDURE proc_xmltype(p_out INOUT xml) "//
                + "LANGUAGE plpgsql AS $$ "                                 //
                + "BEGIN "                                                  //
                + "  p_out := '<xml>abc</xml>'::xml; "                      //
                + "END; "                                                   //
                + "$$;");
    }
    ...
    public void testSqlXmlTypeHandler_1() throws SQLException {
        try (Connection c = DsUtils.pgConn()) {
            ...
            jdbcTemplate.executeUpdate("delete from tb_postgre_types;");

            jdbcTemplate.executeUpdate("insert into tb_postgre_types (c_xml) values ('<xml>abc</xml>')");
            List<String> dat = jdbcTemplate.queryForList("select c_xml from tb_postgre_types where c_xml is not null", (rs, rowNum) -> {
    ...
    public void testSqlXmlTypeHandler_3() throws SQLException {
        try (Connection c = DsUtils.pgConn()) {
            ...
            List<String> dat = jdbcTemplate.queryForList("select ?::xml as c_xml", ps -> {
                new SqlXmlTypeHandler().setParameter(ps, 1, "<xml>abc</xml>", JDBCType.SQLXML.getVendorTypeNumber());
            }, (rs, rowNum) -> {
                return new SqlXmlTypeHandler().getNullableResult(rs, "c_xml");
            });
    ...
    public void testSqlXmlTypeHandler_4() throws SQLException {
        try (Connection c = DsUtils.pgConn()) {
            ...
            Map<String, Object> objectMap = jdbcTemplate.call("call proc_xmltype(?)",//
                    SqlArg.asInOut("out", null, JDBCType.SQLXML.getVendorTypeNumber(), new SqlXmlTypeHandler()));
    ...
    public void testSqlXmlForInputStreamTypeHandler_1() throws Exception {
        try (Connection c = DsUtils.pgConn()) {
            ...
            jdbcTemplate.executeUpdate("insert into tb_postgre_types (c_xml) values ('<xml>abc</xml>')");
            List<InputStream> dat = jdbcTemplate.queryForList("select c_xml from tb_postgre_types where c_xml is not null", (rs, rowNum) -> {
    ...
    public void testSqlXmlForInputStreamTypeHandler_3() throws Exception {
        try (Connection c = DsUtils.pgConn()) {
            ...
            List<InputStream> dat = jdbcTemplate.queryForList("select ?::xml as c_xml", ps -> {
                ...
                return new SqlXmlAsInputStreamTypeHandler().getNullableResult(rs, "c_xml");
            });
            String xmlBody = IOUtils.readToString(dat.get(0), "UTF-8");
            assert xmlBody.trim().equals("<xml>abc</xml>");
        }
    ...
    public void testSqlXmlForInputStreamTypeHandler_4() throws Exception {
        try (Connection c = DsUtils.pgConn()) {
            ...
            Map<String, Object> objectMap = jdbcTemplate.call("call proc_xmltype(?)",//
                    SqlArg.asInOut("out", null, JDBCType.SQLXML.getVendorTypeNumber(), new SqlXmlAsInputStreamTypeHandler()));
    ...
    public void testSqlXmlForReaderTypeHandler_1() throws Exception {
        try (Connection c = DsUtils.pgConn()) {
            ...
            jdbcTemplate.executeUpdate("insert into tb_postgre_types (c_xml) values ('<xml>abc</xml>')");
            List<Reader> dat = jdbcTemplate.queryForList("select c_xml from tb_postgre_types where c_xml is not null", (rs, rowNum) -> {
    ...
    public void testSqlXmlForReaderTypeHandler_3() throws Exception {
        try (Connection c = DsUtils.pgConn()) {
            ...
            jdbcTemplate.executeUpdate("insert into tb_postgre_types (c_xml) values (?)", ps -> {
    ...
    public void testSqlXmlForReaderTypeHandler_4() throws Exception {
        try (Connection c = DsUtils.pgConn()) {
            ...
            Map<String, Object> objectMap = jdbcTemplate.call("call proc_xmltype(?)",//
                    SqlArg.asInOut("out", null, JDBCType.SQLXML.getVendorTypeNumber(), new SqlXmlAsReaderTypeHandler()));
