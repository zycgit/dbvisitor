...
public class SqlCommandBuilderTest {
    ...
    public void testInsert() throws SQLException {
        ...
        Mockito.when(dialect.tableName(Mockito.anyBoolean(), Mockito.any(), Mockito.any(), Mockito.anyString())).thenAnswer(inv -> inv.getArgument(3));
        Mockito.when(dialect.fmtName(Mockito.anyBoolean(), Mockito.anyString())).thenAnswer(inv -> inv.getArgument(1));

        BoundSql boundSql = builder.buildInsert(dialect, false, Collections.emptyList(), DuplicateKeyStrategy.Into);
    ...
    public void testDelete() throws SQLException {
        ...
        BoundSql boundSql = builder.buildDelete(new MySqlDialect(), false, false);
    ...
    public void testOrderByNulls() throws SQLException {
        SqlCommandBuilder builder = new SqlCommandBuilder();
        builder.setTable(null, null, "user_table");
        builder.addOrderBy("name", null, OrderType.ASC, net.hasor.dbvisitor.lambda.core.OrderNullsStrategy.FIRST);

        SqlDialect dialect = Mockito.mock(SqlDialect.class);
        Mockito.when(dialect.tableName(Mockito.anyBoolean(), Mockito.any(), Mockito.any(), Mockito.anyString())).thenAnswer(inv -> inv.getArgument(3));
        Mockito.when(dialect.fmtName(Mockito.anyBoolean(), Mockito.anyString())).thenAnswer(inv -> inv.getArgument(1));
        Mockito.when(dialect.orderByNulls(Mockito.anyBoolean(), Mockito.anyString(), Mockito.nullable(String.class), Mockito.any())).thenReturn("name ASC NULLS FIRST");
        Mockito.when(dialect.orderByDefault(Mockito.anyBoolean(), Mockito.anyString(), Mockito.nullable(String.class), Mockito.any())).thenReturn("name ASC");

        BoundSql boundSql = builder.buildSelect(dialect, false);
        assertEquals("SELECT * FROM user_table ORDER BY name ASC NULLS FIRST, name ASC", boundSql.getSqlString());
    }
}
