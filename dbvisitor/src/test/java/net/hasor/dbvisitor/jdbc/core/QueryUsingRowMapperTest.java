...
public class QueryUsingRowMapperTest {
    ...
    public void usingPosArgs_1() throws Throwable {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            List<UserInfo2> users = jdbcTemplate.queryForList("select * from user_info where user_uuid = ?", user.getUserUuid(), new MappingRowMapper<>(UserInfo2.class));
            assert users.size() == 1;
    ...
    public void usingPosArgs_2() throws SQLException {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            List<UserInfo2> users = jdbcTemplate.queryForList("select * from user_info where user_uuid = ?", args, new MappingRowMapper<>(UserInfo2.class));
            assert users.size() == 1;
            assert user.getUserUuid().equals(users.get(0).getUid());
        }
    }
    ...
    public void usingRuleArgs_1() throws SQLException {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            Map<String, String> mapParams = new HashMap<>();
            mapParams.put("uuid", user.getUserUuid());
            List<UserInfo2> users = jdbcTemplate.queryForList("select * from user_info where user_uuid = @{arg,true,uuid}", mapParams, new MappingRowMapper<>(UserInfo2.class));
            assert users.size() == 1;
            assert user.getUserUuid().equals(users.get(0).getUid());
        }
    }
    ...
    public void argtype_as_source_1() throws SQLException {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            List<UserInfo2> users = jdbcTemplate.queryForList("select * from user_info where user_uuid = :uuid", source, new MappingRowMapper<>(UserInfo2.class));
            assert users.size() == 1;
            assert user.getUserUuid().equals(users.get(0).getUid());
        }
    }
