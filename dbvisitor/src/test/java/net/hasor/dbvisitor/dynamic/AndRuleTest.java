...
public class AndRuleTest {
    ...
    public void ruleTest_1() throws SQLException {
        ...
        //
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("@{and,name = :name}").buildQuery(ctx, new DynamicContext());
        ...
        // more space char
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("@{and, name = :name}").buildQuery(ctx, new DynamicContext());
        ...
        //
        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("@{and,name = ?}").buildQuery(ctx, new DynamicContext());
        ...
        // more space char
        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("@{and, name = ?}").buildQuery(ctx, new DynamicContext());
    ...
    public void ruleTest_2() throws SQLException {
        ...
        //
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("where  @{and,name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("where  name = ?");
        assert sqlBuilder1.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder1.getArgs()[0]).getValue().equals("abc");

        // more space char
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("where  @{and, name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder2.getSqlString().equals("where   name = ?");
        assert sqlBuilder2.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder2.getArgs()[0]).getValue().equals("abc");

        //
        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("where  @{and,name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder3.getSqlString().equals("where  name = ?");
        assert sqlBuilder3.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder3.getArgs()[0]).getValue().equals("123");

        // more space char
        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("where  @{and, name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder4.getSqlString().equals("where   name = ?");
        assert sqlBuilder4.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder4.getArgs()[0]).getValue().equals("123");
    }
    ...
    public void ruleTest_3() throws SQLException {
        ...
        //
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("where 1=1 @{and,name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("where 1=1 and name = ?");
        assert sqlBuilder1.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder1.getArgs()[0]).getValue().equals("abc");

        // more space char
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("where 1=1 @{and, name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder2.getSqlString().equals("where 1=1 and  name = ?");
        assert sqlBuilder2.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder2.getArgs()[0]).getValue().equals("abc");

        //
        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("where 1=1 @{and,name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder3.getSqlString().equals("where 1=1 and name = ?");
        assert sqlBuilder3.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder3.getArgs()[0]).getValue().equals("123");

        // more space char
        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("where 1=1 @{and, name = ?}").buildQuery(ctx, new DynamicContext());
    ...
    public void ruleTest_5() throws SQLException {
        ...
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("@{and}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("");
        assert sqlBuilder1.getArgs().length == 0;

        //
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("@{and,abc}").buildQuery(ctx, new DynamicContext());
    ...
    public void ifruleTest_1() throws SQLException {
        ...
        //
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("@{ifand,test,name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("where name = ?");
        assert sqlBuilder1.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder1.getArgs()[0]).getValue().equals("abc");

        // more space char
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("@{ifand,test, name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder2.getSqlString().equals("where  name = ?");
        assert sqlBuilder2.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder2.getArgs()[0]).getValue().equals("abc");

        //
        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("@{ifand,test,name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder3.getSqlString().equals("where name = ?");
        assert sqlBuilder3.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder3.getArgs()[0]).getValue().equals("123");

        // more space char
        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("@{ifand,test, name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder4.getSqlString().equals("where  name = ?");
        assert sqlBuilder4.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder4.getArgs()[0]).getValue().equals("123");
    }
    ...
    public void ifruleTest_2() throws SQLException {
        ...
        //
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("where  @{ifand,test,name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("where  name = ?");
        assert sqlBuilder1.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder1.getArgs()[0]).getValue().equals("abc");

        // more space char
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("where  @{ifand,test, name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder2.getSqlString().equals("where   name = ?");
        assert sqlBuilder2.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder2.getArgs()[0]).getValue().equals("abc");

        //
        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("where  @{ifand,test,name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder3.getSqlString().equals("where  name = ?");
        assert sqlBuilder3.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder3.getArgs()[0]).getValue().equals("123");

        // more space char
        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("where  @{ifand,test, name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder4.getSqlString().equals("where   name = ?");
        assert sqlBuilder4.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder4.getArgs()[0]).getValue().equals("123");
    }
    ...
    public void ifruleTest_3() throws SQLException {
        ...
        //
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("where 1=1 @{ifand,test,name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("where 1=1 and name = ?");
        assert sqlBuilder1.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder1.getArgs()[0]).getValue().equals("abc");

        // more space char
        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("where 1=1 @{ifand,test, name = :name}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder2.getSqlString().equals("where 1=1 and  name = ?");
        assert sqlBuilder2.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder2.getArgs()[0]).getValue().equals("abc");

        //
        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("where 1=1 @{ifand,test,name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder3.getSqlString().equals("where 1=1 and name = ?");
        assert sqlBuilder3.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder3.getArgs()[0]).getValue().equals("123");

        // more space char
        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("where 1=1 @{ifand,test, name = ?}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder4.getSqlString().equals("where 1=1 and  name = ?");
        assert sqlBuilder4.getArgs().length == 1;
        assert ((MappedArg) sqlBuilder4.getArgs()[0]).getValue().equals("123");

        SqlBuilder sqlBuilder5 = DynamicParsed.getParsedSql("where name = :name and  @{ifand,test, name = ?}").buildQuery(ctx, new DynamicContext());
    ...
    public void ifruleTest_5() throws SQLException {
        ...
        SqlBuilder sqlBuilder1 = DynamicParsed.getParsedSql("@{ifand}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder1.getSqlString().equals("");
        assert sqlBuilder1.getArgs().length == 0;

        SqlBuilder sqlBuilder2 = DynamicParsed.getParsedSql("@{ifand,test}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder2.getSqlString().equals("");
        assert sqlBuilder2.getArgs().length == 0;

        SqlBuilder sqlBuilder3 = DynamicParsed.getParsedSql("@{ifand,abc}").buildQuery(ctx, new DynamicContext());
        assert sqlBuilder3.getSqlString().equals("");
        assert sqlBuilder3.getArgs().length == 0;

        SqlBuilder sqlBuilder4 = DynamicParsed.getParsedSql("@{ifand,test,abc}").buildQuery(ctx, new DynamicContext());
