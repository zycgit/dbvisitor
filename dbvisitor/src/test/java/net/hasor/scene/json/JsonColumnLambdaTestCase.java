...
public class JsonColumnLambdaTestCase {
    ...
    public void insertTestCase() throws SQLException, IOException {
        try (Connection c = DsUtils.mysqlConn()) {
            ...
            Project1 result = template.lambdaQuery(Project1.class).eq(Project1::getId, 1).queryForObject();
    ...
    public void updateTestCase() throws SQLException, IOException {
        try (Connection c = DsUtils.mysqlConn()) {
            ...
            Project1 project = newProject("abc", Arrays.asList("CN", "EN"));
            template.lambdaInsert(Project1.class).applyEntity(project).executeSumResult();

            ProjectFeature feature = new ProjectFeature();
            feature.setDesc("this is desc2");
            feature.setTags(Arrays.asList("JP", "FR"));
            assert 1 == template.lambdaUpdate(Project1.class).eq(Project1::getId, 1).updateTo(Project1::getFeature, feature).doUpdate();
    ...
    public void whereTestCase() throws SQLException, IOException {
        try (Connection c = DsUtils.mysqlConn()) {
            ...
            template.lambdaInsert(Project1.class).applyEntity(project1).executeSumResult();
            template.lambdaInsert(Project1.class).applyEntity(project2).executeSumResult();

            ProjectFeature feature = project2.getFeature();
            Project1 result = template.lambdaQuery(Project1.class).eq(Project1::getFeature, feature).queryForObject();
