...
public class PrimitiveTestCase {
    ...
    public void stringTestCase() throws SQLException {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            template.lambdaDelete(PrimitiveDTO.class).allowEmptyWhere().doDelete();

            PrimitiveDTO dto = new PrimitiveDTO();
            dto.setId(1);
            dto.setIntValue(1);
            dto.setVarcharValue("string 1");
            template.lambdaInsert(PrimitiveDTO.class).applyEntity(dto).executeSumResult();
            dto.setId(2);
            dto.setIntValue(2);
            ...
            // using sql
            List<String> strings1 = template.queryForList("select c_varchar from tb_h2_types order by id asc", String.class);
            assert strings1.get(0).equals("string 1");
            assert strings1.get(1).equals("string 2");

            // using lambda
            List<String> strings2 = template.lambdaQuery(PrimitiveDTO.class)//
    ...
    public void intTestCase() throws SQLException {
        try (Connection c = DsUtils.h2Conn()) {
            ...
            // using sql
            List<Integer> ints1 = template.queryForList("select c_int from tb_h2_types order by id asc", Integer.class);
            assert ints1.get(0).equals(10);
            assert ints1.get(1).equals(20);

            // using lambda
            List<Integer> ints2 = template.lambdaQuery(PrimitiveDTO.class)//
