...
public class BlockFileAdapter {
    ...
    private              AtomicLong       currentStreamID = null;
    ...
    public BlockFileAdapter(File blockFileName, int bufferSize) throws IOException {
        ...
        this.currentStreamID = new AtomicLong(0);
    ...
    public boolean releaseStream() {
        return this.releaseStream(this.currentStreamID.get());
    }

    private boolean releaseStream(long streamID) {
        if (streamID <= 0 || this.currentStreamID.get() != streamID) {
            return false;
        }
        return currentStreamID.compareAndSet(streamID, 0);
    }
    ...
    public Block nextBlock(Block atBlock) throws IOException {
        ...
        Block curentBlock = (atBlock != null) ? atBlock : this.currentBlock;
        if (curentBlock == null) {
            ioSeekTo(0);
        } else {
    ...
    public boolean hasNext() throws IOException {
        if (this.currentBlock != null) {
            return (this.currentBlock.getPosition() + this.currentBlock.stiffBlockSize()) != this.fileSize();
        }
    ...
    public InputStream getInputStream(Block block) throws IOException {
        ...
        if (this.currentStreamID.compareAndSet(0, streamID)) {
    ...
    private void checkStreamClose(boolean isClose, long streamID) throws IOException {
        ...
        if (streamID != currentStreamID.get()) {
