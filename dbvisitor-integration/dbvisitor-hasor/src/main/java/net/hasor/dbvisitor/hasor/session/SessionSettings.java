/*
 * Copyright 2015-2022 the original author or authors.
 *
 * Licensed under the Apache License, Version 2.0.
 * See the LICENSE.txt file for the full license.
 * https://www.apache.org/licenses/LICENSE-2.0
 */
package net.hasor.dbvisitor.hasor.session;
import net.hasor.cobble.StringUtils;
import net.hasor.cobble.setting.Settings;

final class SessionSettings {
    private final Settings settings;
    private final String   sourceName;

    SessionSettings(Settings settings, String sourceName) {
        this.settings = settings;
        this.sourceName = sourceName;
    }

    String getMapperLocations() {
        return this.getQualified("mapper-locations", "dbvisitor/mapper/*.xml");
    }

    String getAutoMapping() {
        return this.getOption("auto-mapping");
    }

    String getCamelCase() {
        return this.getOption("camel-case");
    }

    String getCaseInsensitive() {
        return this.getOption("case-insensitive");
    }

    String getUseDelimited() {
        return this.getOption("use-delimited");
    }

    String getIgnoreNonExistStatement() {
        return this.getOption("ignore-nonexist-statement");
    }

    String getSqlDialect() {
        return this.getOption("dialect");
    }

    private String getOption(String name) {
        String value = this.getString(this.buildKey(name), null);
        return StringUtils.isBlank(value) ? this.getString(name, null) : value;
    }

    private String getQualified(String name, String defaultValue) {
        return this.getString(this.buildKey(name), defaultValue);
    }

    private String buildKey(String name) {
        return StringUtils.isBlank(this.sourceName) ? "dbvisitor." + name : "dbvisitor." + this.sourceName + "." + name;
    }

    private String getString(String key, String defaultValue) {
        String value = this.settings.getString(key, null);
        if (StringUtils.isNotBlank(value)) {
            return value;
        }
        String underscoreKey = key.replace('-', '_');
        if (!underscoreKey.equals(key)) {
            value = this.settings.getString(underscoreKey, null);
            if (StringUtils.isNotBlank(value)) {
                return value;
            }
        }
        String camelKey = this.keyToCamelPath(key);
        if (!camelKey.equals(key)) {
            value = this.settings.getString(camelKey, null);
            if (StringUtils.isNotBlank(value)) {
                return value;
            }
        }
        return defaultValue;
    }

    private String keyToCamelPath(String key) {
        String[] segments = key.split("\\.");
        for (int i = 0; i < segments.length; i++) {
            if (segments[i].indexOf('-') >= 0) {
                segments[i] = StringUtils.lineToHump(segments[i].replace('-', '_'));
            }
        }
        return String.join(".", segments);
    }
}
