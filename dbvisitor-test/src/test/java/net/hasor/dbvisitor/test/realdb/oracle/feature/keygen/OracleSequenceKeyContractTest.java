...
public class OracleSequenceKeyContractTest extends SequenceKeyContractTest {
    @Override
    ...
    public void sequence_shouldUseExplicitSchema() throws SQLException {
        ...
        String schema = jdbcTemplate.queryForObject("SELECT SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') FROM dual", String.class);
    ...
    public void sequence_shouldPreserveQuotedNames() throws SQLException {
        dropTableIfExists("\"OracleSequenceUser\"");
        resetSequence("\"OracleUserSeq\"", 5000);
        try {
            jdbcTemplate.executeUpdate("CREATE TABLE \"OracleSequenceUser\" (\"id\" NUMBER(19) PRIMARY KEY, \"name\" VARCHAR2(100))");
            String schema = jdbcTemplate.queryForObject("SELECT SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') FROM dual", String.class);
            LambdaTemplate template = new LambdaTemplate(dataSource, Options.of().dialect(sequenceDialect()).schema(schema));
            ...
            assertEquals(user.getName(), jdbcTemplate.queryForObject("SELECT \"name\" FROM \"OracleSequenceUser\" WHERE \"id\" = ?", new Object[] { user.getId() }, String.class));
        } finally {
            dropTableIfExists("\"OracleSequenceUser\"");
            dropSequenceIfExists("\"OracleUserSeq\"");
        }
    }

    @Table(value = "OracleSequenceUser", useDelimited = true)
    public static class QuotedSequenceUser {
        @Column(primary = true, keyType = KeyType.Sequence)
        @KeySeq("OracleUserSeq")
        private Long id;
        private String name;
