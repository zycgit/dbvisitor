...
public abstract class JdbcResultExtractorCase extends JdbcResultHandlingSupport {
    // 能力归属：结果接收 / resultsetextractor / jdbc。
    @Test
    @Capability(value = CapabilityId.JDBC_RESULT_EXTRACTOR_CUSTOM, column = "results/resultsetextractor/extraction", variants = { "jdbc" })
    public void resultSetExtractor_shouldSupportCustomAggregation() throws SQLException {
        ...
        Map<Object, Object> result = ResultHandlerProbe.verify(1, () -> jdbcTemplate.query(selectSql("id, name", "id BETWEEN ? AND ?", true), //
                selectArguments("id, name", "id BETWEEN ? AND ?", baseId() + 1, baseId() + 3), extractor));

        assertEquals(3, result.size());
        assertEquals(expectedCustomMap(), result);
        assertTrue(ResultHandlerProbe.verify(1, () -> jdbcTemplate.query(emptyResultSql(), emptyResultArguments(), extractor)).isEmpty());
        ResultHandlerProbe.verifyFailure(() -> jdbcTemplate.query(selectSql("id, name", "id BETWEEN ? AND ?", true), //
                selectArguments("id, name", "id BETWEEN ? AND ?", baseId() + 1, baseId() + 3), extractor));
    }

    // 能力归属：结果接收 / resultsetextractor / jdbc。
    @Test
    @Capability(value = CapabilityId.JDBC_RESULT_EXTRACTOR_BUILTIN, column = "results/resultsetextractor/extraction", variants = { "jdbc" })
    public void resultSetExtractor_shouldSupportBuiltInListExtractors() throws SQLException {
        ...
        List<?> filteredRows = jdbcTemplate.query(selectSql("*", "id BETWEEN ? AND ?", true), //
                selectArguments("*", "id BETWEEN ? AND ?", baseId() + 1, baseId() + 10), new FilterResultSetExtractor<>(rowMapper, row -> ((Number) beanProperty(row, filterNumberProperty())).doubleValue() > 24));
        ...
        assertEquals(List.of(25.0, 26.0, 27.0, 28.0, 29.0, 30.0), filteredRows.stream()//
                .map(row -> ((Number) beanProperty(row, filterNumberProperty())).doubleValue()).toList());
    }

    // 能力归属：结果接收 / resultsetextractor / jdbc。
    @Test
    @Capability(value = CapabilityId.JDBC_RESULT_EXTRACTOR_PAIRS, column = "results/resultsetextractor/extraction", variants = { "jdbc" })
    public void resultSetExtractor_shouldSupportPairsExtractor() throws SQLException {
        ...
        assertEquals(result, jdbcTemplate.queryForPairs(selectSql("id, name", "id BETWEEN ? AND ?", true), pairKeyType(), pairValueType(), //
                selectArguments("id, name", "id BETWEEN ? AND ?", baseId() + 7, baseId() + 9)));
        Map<?, ?> keysOnly = jdbcTemplate.queryForPairs(selectSql("id", "id BETWEEN ? AND ?", true), pairKeyType(), pairValueType(), //
                selectArguments("id", "id BETWEEN ? AND ?", baseId() + 7, baseId() + 9));
        assertEquals(new ArrayList<>(result.keySet()), new ArrayList<>(keysOnly.keySet()));
        assertTrue(keysOnly.values().stream().allMatch(Objects::isNull));
        assertTrue(jdbcTemplate.queryForPairs(emptyResultSql(), pairKeyType(), pairValueType(), emptyResultArguments()).isEmpty());
    }

    // 能力归属：结果接收 / resultsetextractor / jdbc。
    @Test
    @Capability(value = CapabilityId.JDBC_RESULT_EXTRACTOR_MAPPING, column = "results/resultsetextractor/extraction", variants = { "jdbc" })
    ...
    protected Map<String, Object> expectedMappedRow() {
        return Map.of("id", baseId() + 6, "name", "NXN-Result-6", "age", 26, "email", "nxn-result-6@test.com");
    }

    protected Map<String, Object> expectedMappingBean() {
        return Map.of("id", baseId() + 5, "name", "NXN-Result-5", "age", 25, "email", "nxn-result-5@test.com");
    }

    protected Map<String, Object> expectedMappingMap() {
        return Map.of("id", baseId() + 10, "name", "NXN-Result-10", "age", 30, "email", "nxn-result-10@test.com");
    }
}
