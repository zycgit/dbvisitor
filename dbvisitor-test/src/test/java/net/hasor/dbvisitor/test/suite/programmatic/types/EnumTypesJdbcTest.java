...
import net.hasor.dbvisitor.test.model.types.StatusEnum;
import net.hasor.dbvisitor.test.model.types.StatusEnumOfCode;
import net.hasor.dbvisitor.test.model.types.StatusEnumOfValue;
 ...
 * 5. null 值处理
 * 6. 无效值处理
 * 数据库初始化：通过 /sql/{dialect}/init.sql 脚本自动创建 enum_types_explicit_test 表
 * 重点：JdbcTemplate 通过 EnumTypeHandler 支持直接使用枚举类型作为查询结果类型
 * 枚举类型定义位于：net.hasor.dbvisitor.test.model.types
 */
