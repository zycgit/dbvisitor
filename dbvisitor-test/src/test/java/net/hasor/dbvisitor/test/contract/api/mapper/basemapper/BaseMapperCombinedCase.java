...
@NxnContract
public abstract class BaseMapperCombinedCase extends BaseMapperCrudSupport {
    // 能力归属：Mapper API / Mapper 读写。
    @Test
    @Capability(value = CapabilityId.BASEMAPPER_RESULT_BATCH, column = "mapper/base-mapper/operations")
    ...
    // 能力归属：Mapper API / Mapper 读写。
    @Test
    @Capability(value = CapabilityId.BASEMAPPER_PARAMETER_BATCH, column = "mapper/base-mapper/operations")
    ...
    @Test
    @Capability(value = CapabilityId.BASEMAPPER_MIXED_OPERATIONS, column = "mapper/builder/calls")
    ...
    protected void verifyJdbcAndLambdaAccessors() throws SQLException {
        ...
        int jdbcResult = this.mapper.jdbc().executeUpdate(accessorInsertCommand(), new Object[] { jdbcId, "BaseAccessorJdbc", 142, new Date() });
