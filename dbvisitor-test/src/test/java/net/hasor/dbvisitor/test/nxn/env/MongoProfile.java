...
public final class MongoProfile extends AbstractDataSourceProfile {
    ...
    private MongoProfile() {
        super(FeatureId.TRANSACTION, FeatureId.TRANSACTION_SAVEPOINT, FeatureId.TRANSACTION_RELEASE_SAVEPOINT,
                ...
                FeatureId.FUNCTION_RECORD_RESULT, FeatureId.FUNCTION_TABLE_RESULT, FeatureId.FUNCTION_CALL_CALLBACK,
                FeatureId.SEQUENCE, FeatureId.GENERATED_KEYS_NUMERIC, FeatureId.KEYGEN_AUTO_BATCH_EXPLICIT_NULL,
                FeatureId.GENERATED_KEY_RESULT_SET, FeatureId.KNN, FeatureId.SQL_NOT_IN_NULL_SEMANTICS);
    }
    ...
    public SupportStatus support(String capabilityId) {
        ...
        if (CapabilityId.MAPPER_XML_KEYGEN_RESULT_SET_SOURCE.equals(capabilityId)
                || CapabilityId.MAPPER_ANNOTATION_ATTRIBUTE_RESULT_SET_KEY_SOURCE.equals(capabilityId)) {
            return SupportStatus.UNSUPPORTED_BY_DRIVER;
        }
        // These shared APIs use real ObjectId/String models, not numeric IDENTITY fixtures.
        if (CapabilityId.MAPPER_ANNOTATION_ATTRIBUTE_GENERATED_KEYS.equals(capabilityId)
                ...
                || CapabilityId.KEYGEN_HOLDER_AFTER.equals(capabilityId)) {
        ...
        if (capabilityId.startsWith("vector.")
                && !CapabilityId.VECTOR_CRUD_ROUND_TRIP.equals(capabilityId)
                && !CapabilityId.VECTOR_BATCH_QUERY.equals(capabilityId)
                && !CapabilityId.VECTOR_CRUD_DELETE_AND_SCALAR_UPDATE.equals(capabilityId)
                && !CapabilityId.VECTOR_PRECISION_BOUNDARY.equals(capabilityId)) {
            return SupportStatus.UNSUPPORTED_BY_DATABASE;
        }
        // SQL-generating rules are distinct from native BSON collection binding.
        if (CapabilityId.JDBC_PARAM_RULE_SET.equals(capabilityId) || CapabilityId.JDBC_PARAM_RULE_AND_IN_SET.equals(capabilityId)
