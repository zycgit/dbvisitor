...
public class OracleProcedureContractTest extends AbstractProcedureContractTest {
    @Override
    ...
    protected void createProcedureDefinitions() throws SQLException {
        ...
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE nxn_sp_transform_string(text_value IN OUT VARCHAR2, suffix IN VARCHAR2) AS "
                + "BEGIN text_value := UPPER(text_value) || suffix; END;");
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE nxn_sp_get_user_info(user_id IN NUMBER, user_name IN OUT VARCHAR2, user_age IN OUT NUMBER) AS "
        ...
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE nxn_sp_cursor_users(p_name IN VARCHAR2, res OUT SYS_REFCURSOR) AS "
