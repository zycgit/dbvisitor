...
public class RedisJdbcArgumentSourceParameterTest extends JdbcArgumentSourceParameterCase {
    ...
    protected String command(JdbcParameterCommand command) {
        ...
        String insert = "EVAL 'return redis.call(\"HSET\", KEYS[1], ARGV[1] .. \":name\", ARGV[2], " + "ARGV[2] .. \":age\", ARGV[3], ARGV[1] .. \":email\", ARGV[4], ARGV[1] .. \":created\", ARGV[5])' 1 " + key;
        switch (command) {
            ...
            case COUNT_SOURCE_NAMES:
                return "EVAL 'local n = 0; for i = 1, 3 do local age = redis.call(\"HGET\", KEYS[1], ARGV[i] .. \":age\"); " + "if age and tonumber(age) > tonumber(ARGV[4]) then n = n + 1 end end return n' 1 " + key + " #{names[0]} #{names[1]} #{names[2]} :minAge";
