 ...
 * 测试范围：
 * 1. StatusEnum name() 映射到 VARCHAR 字段（LambdaTemplate CRUD）
 * 2. StatusEnumOfCode 自定义字符串映射（LambdaTemplate + EnumOfCodeModel）
 * 3. StatusEnumOfValue 数值映射（LambdaTemplate + EnumOfValueModel）
 * 4. null 值处理
 * 5. 更新操作
 * 6. 无效值处理
 */
public class EnumTypesFluentTest extends AbstractOneApiTest {
    ...
    /** EnumOfCode 测试用 Bean，映射 status_string 列为 StatusEnumOfCode */
    @Table("enum_types_explicit_test")
    public static class EnumOfCodeModel {
        @Column(name = "id", primary = true)
        private Integer id;
        @Column(name = "status_string")
        private StatusEnumOfCode statusString;

        public Integer getId() { return id; }

        public void setId(Integer id) { this.id = id; }

        public StatusEnumOfCode getStatusString() { return statusString; }

        public void setStatusString(StatusEnumOfCode statusString) { this.statusString = statusString; }
    }

    /** EnumOfValue 测试用 Bean，映射 status_ordinal 列为 StatusEnumOfValue */
    @Table("enum_types_explicit_test")
    public static class EnumOfValueModel {
        @Column(name = "id", primary = true)
        private Integer id;
        ...
        public StatusEnumOfValue getStatusOrdinal() { return statusOrdinal; }

        public void setStatusOrdinal(StatusEnumOfValue statusOrdinal) { this.statusOrdinal = statusOrdinal; }
    }
    ...
    /**
     * 测试 EnumOfCode - LambdaTemplate 通过 EnumOfCodeModel 验证自定义字符串映射
    ...
    public void testEnumOfCode_CustomStringMapping() throws SQLException {
        ...
        EnumOfCodeModel model1 = new EnumOfCodeModel();
        model1.setId(7);
        model1.setStatusString(StatusEnumOfCode.ACTIVE);

        EnumOfCodeModel model2 = new EnumOfCodeModel();
        model2.setId(8);
        model2.setStatusString(StatusEnumOfCode.INACTIVE);

        EnumOfCodeModel model3 = new EnumOfCodeModel();
        model3.setId(9);
        model3.setStatusString(StatusEnumOfCode.DELETED);

        lambdaTemplate.insert(EnumOfCodeModel.class).applyEntity(model1).executeSumResult();
        lambdaTemplate.insert(EnumOfCodeModel.class).applyEntity(model2).executeSumResult();
        lambdaTemplate.insert(EnumOfCodeModel.class).applyEntity(model3).executeSumResult();

        // 通过 LambdaTemplate 查询验证
        EnumOfCodeModel loaded1 = lambdaTemplate.query(EnumOfCodeModel.class)//
                .eq(EnumOfCodeModel::getId, 7)//
                .queryForObject();
        assertNotNull(loaded1);
        assertEquals(StatusEnumOfCode.ACTIVE, loaded1.getStatusString());
        assertEquals("active", loaded1.getStatusString().codeName());

        EnumOfCodeModel loaded2 = lambdaTemplate.query(EnumOfCodeModel.class)//
                .eq(EnumOfCodeModel::getId, 8)//
                .queryForObject();
        assertEquals(StatusEnumOfCode.INACTIVE, loaded2.getStatusString());
        assertEquals("inactive", loaded2.getStatusString().codeName());

        EnumOfCodeModel loaded3 = lambdaTemplate.query(EnumOfCodeModel.class)//
                .eq(EnumOfCodeModel::getId, 9)//
                .queryForObject();
        assertEquals(StatusEnumOfCode.DELETED, loaded3.getStatusString());
        assertEquals("deleted", loaded3.getStatusString().codeName());
    }

    /**
     * 测试 EnumOfValue - LambdaTemplate 通过 EnumOfValueModel 验证数值映射
    ...
    public void testEnumOfValue_IntegerMapping() throws SQLException {
        // 通过 LambdaTemplate 插入 EnumOfValue 枚举值
        EnumOfValueModel model1 = new EnumOfValueModel();
        model1.setId(10);
        model1.setStatusOrdinal(StatusEnumOfValue.ACTIVE);

        EnumOfValueModel model2 = new EnumOfValueModel();
        model2.setId(11);
        model2.setStatusOrdinal(StatusEnumOfValue.INACTIVE);

        EnumOfValueModel model3 = new EnumOfValueModel();
        model3.setId(12);
        model3.setStatusOrdinal(StatusEnumOfValue.DELETED);

        lambdaTemplate.insert(EnumOfValueModel.class).applyEntity(model1).executeSumResult();
        lambdaTemplate.insert(EnumOfValueModel.class).applyEntity(model2).executeSumResult();
        lambdaTemplate.insert(EnumOfValueModel.class).applyEntity(model3).executeSumResult();

        // 通过 LambdaTemplate 查询验证
        EnumOfValueModel loaded1 = lambdaTemplate.query(EnumOfValueModel.class)//
                .eq(EnumOfValueModel::getId, 10)//
                .queryForObject();
        assertNotNull(loaded1);
        assertEquals(StatusEnumOfValue.ACTIVE, loaded1.getStatusOrdinal());
        assertEquals(1, loaded1.getStatusOrdinal().codeValue());

        EnumOfValueModel loaded2 = lambdaTemplate.query(EnumOfValueModel.class)//
                .eq(EnumOfValueModel::getId, 11)//
                .queryForObject();
        assertEquals(StatusEnumOfValue.INACTIVE, loaded2.getStatusOrdinal());
        assertEquals(0, loaded2.getStatusOrdinal().codeValue());

        EnumOfValueModel loaded3 = lambdaTemplate.query(EnumOfValueModel.class)//
                .eq(EnumOfValueModel::getId, 12)//
                .queryForObject();
