...
public abstract class JdbcRuleParameterCase extends JdbcParameterSupport {
    ...
    @Test
    @Capability(value = CapabilityId.JDBC_PARAM_RULE_AND_IN_SET, column = "parameters/command-rules/sql-fragments")
    ...
    // 能力归属：参数传递与规则 / SQL 片段规则。
    @Test
    @Capability(value = CapabilityId.JDBC_PARAM_RULE_SET, column = "parameters/command-rules/sql-fragments")
    public void ruleParameters_shouldExpandSetClauses() throws SQLException {
        ...
        assertNull(readEmail(command(JdbcParameterCommand.SELECT_EMAIL_BY_ID), new Object[] { baseId() + 10 }));
    }
}
