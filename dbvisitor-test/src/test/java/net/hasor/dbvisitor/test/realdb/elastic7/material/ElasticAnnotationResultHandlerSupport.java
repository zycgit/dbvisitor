...
public abstract class ElasticAnnotationResultHandlerSupport extends AnnotationMapperResultHandlerCase {
    ...
    public void createResultHandlerMapper() throws Exception {
        ...
        configuration.addMacro("esHandlerInsert", path + "/_doc " + "{\"id\": #{id},\"name\": #{name},\"age\": #{age},\"email\": #{email},\"create_time\": #{createTime}}");
        configuration.addMacro("esHandlerAll", path + "/_search {\"_source\": [\"id\",\"name\",\"age\",\"email\",\"create_time\"],\"size\": 100,\"sort\": [{\"id\": \"asc\"}]," + "\"query\": {\"wildcard\": {\"name\": #{pattern.replace('%', '*')}}}}");
    ...
    public interface NativeResultMapper extends ResultHandlerMapper {
        ...
        @Override
        @Query(value = "@{macro, esHandlerAll}", resultSetExtractor = UserNameMapExtractor.class)
