...
public class PostgreSqlXmlMapperCallableContractTest extends AbstractXmlMapperCallableContractTest {
    ...
    protected void createCallableDefinitions() throws SQLException {
        ...
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_user_stats(INOUT p_count INT, INOUT p_max_id INT, INOUT p_min_name VARCHAR) LANGUAGE plpgsql AS $$ BEGIN SELECT COUNT(*), MAX(id) INTO p_count, p_max_id FROM user_info WHERE name LIKE 'XmlCallable%'; SELECT name INTO p_min_name FROM user_info WHERE name LIKE 'XmlCallable%' ORDER BY id LIMIT 1; END; $$");
