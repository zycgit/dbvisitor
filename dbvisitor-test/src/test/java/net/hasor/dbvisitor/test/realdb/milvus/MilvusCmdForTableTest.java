...
public class MilvusCmdForTableTest extends AbstractMilvusCmdForTest {
    ...
    @Test
    public void testDefaultUsedWhenInsertOmitsScalarField() throws Exception {
        try (Connection conn = DriverManager.getConnection(MILVUS_URL); Statement stmt = conn.createStatement()) {
            ...
            try (ResultSet result = stmt.executeQuery("SELECT word_count FROM " + TEST_COLLECTION + " WHERE book_id = 1 LIMIT 1")) {
                ...
                Assert.assertEquals(java.sql.Types.INTEGER, result.getMetaData().getColumnType(1));
    ...
    public static void main(String[] args) {
        net.hasor.dbvisitor.test.realdb.RealDbTestRunner.run(MilvusCmdForTableTest.class);
    }
}
