...
public class XmlMappingMetadataTest {
    @Test
    ...
    public void xmlMapping_shouldExposeLoadBoundaries() throws Exception {
        ...
        blank.loadMapping("");
        blank.loadMapping("  ");
        try (InputStream stream = new ByteArrayInputStream("<mapper/>".getBytes(StandardCharsets.UTF_8))) {
    ...
    private void loadFromResource(MappingRegistry registry, String id, String resource) throws Exception {
        try (InputStream stream = new ByteArrayInputStream(readResource(resource).getBytes(StandardCharsets.UTF_8))) {
