...
public class Elastic7XmlMapperResultHandlerTest extends XmlMapperResultHandlerCase {
    ...
    public void setup() throws SQLException {
        ...
        for (int i = 1; i <= 3; i++) {
            jdbcTemplate.executeUpdate("PUT /" + fixture.index() + "/_doc/" + (baseId() + i)
                    + "?refresh=true {\"id\": ?,\"name\": ?,\"age\": ?,\"email\": ?,\"create_time\": ?}",
                    new Object[] { baseId() + i, "ResHdl" + i, 20 + i * 5, "hdl" + i + "@nxn.test", new Date(timestamp()) });
        }
    }
