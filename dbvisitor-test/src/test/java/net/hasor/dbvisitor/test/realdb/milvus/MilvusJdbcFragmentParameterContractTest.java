...
public class MilvusJdbcFragmentParameterContractTest extends JdbcFragmentParameterContractTest {
    ...
    protected void prepareFragmentOrder() throws SQLException {
        // Distances to [100, 0] give the fixture its expected age order: 34, then 20.
        this.jdbcTemplate.executeUpdate(sql("UPDATE user_info SET v = [20, 0] WHERE id = ?"), new Object[] { baseId() + 8 });
        this.jdbcTemplate.executeUpdate(sql("UPDATE user_info SET v = [34, 0] WHERE id = ?"), new Object[] { baseId() + 9 });
    }

    @Override
    protected String orderFragment() {
        return "v <-> [100, 0] LIMIT 2 WITH (metric_type=L2)";
    }
