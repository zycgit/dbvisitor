...
public class OracleXmlMapperCallableContractTest extends XmlMapperCallableContractTest {
    @Override
    ...
    protected void createCallableDefinitions() throws SQLException {
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_insert_user(p_id IN NUMBER, p_name IN VARCHAR2) AS " + //
                "BEGIN INSERT INTO user_info (id, name, age, create_time) VALUES (p_id, p_name, 0, " + currentTimestampExpression() + "); END;");
        ...
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_multi_inout(p_prefix IN VARCHAR2, p_suffix IN VARCHAR2, p_concat IN OUT VARCHAR2, p_length IN OUT NUMBER) AS " + //
                "BEGIN p_concat := p_prefix || '-' || p_suffix; p_length := LENGTH(p_concat); END;");
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_user_count(p_count IN OUT NUMBER) AS " + //
                "BEGIN SELECT COUNT(*) INTO p_count FROM user_info WHERE name LIKE 'XmlCallable%'; END;");
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_user_stats(p_count IN OUT NUMBER, p_max_id IN OUT NUMBER, p_min_name IN OUT VARCHAR2) AS " + //
                "BEGIN " + //
                ...
                "END;");
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_query_users(p_name IN VARCHAR2, p_out IN OUT VARCHAR2, res1 OUT SYS_REFCURSOR) AS " + //
                "BEGIN p_out := 'found:' || p_name; OPEN res1 FOR SELECT id, name, age, email FROM user_info WHERE name = p_name; END;");
        jdbcTemplate.execute("CREATE OR REPLACE PROCEDURE proc_query_users_multi(p_name IN VARCHAR2, p_out_msg IN OUT VARCHAR2, res_matched OUT SYS_REFCURSOR, res_unmatched OUT SYS_REFCURSOR) AS " + //
                "BEGIN " + //
