...
public abstract class QuotedLowercaseTableSupport extends IdentifierQuotingCase {
    ...
    protected void prepareAutoKeywordColumnFixture() throws SQLException {
        ...
        this.jdbcTemplate.executeUpdate("CREATE TABLE naming_keyword_test (" + primaryKeyColumn("id", "INT") + ", " + qualified("order") + " VARCHAR(100), " + qualified("select") + " VARCHAR(100), name VARCHAR(100))");
    }
    ...
    protected void prepareAutoKeywordTableFixture() throws SQLException {
        ...
        this.jdbcTemplate.executeUpdate("CREATE TABLE " + table + " (" + primaryKeyColumn("id", "INT") + ", name VARCHAR(100), description VARCHAR(200))");
    }
    ...
    protected void prepareDelimitedFixture() throws SQLException {
        ...
        dropTableIfExists(table);
        this.jdbcTemplate.executeUpdate("CREATE TABLE " + table + " (" + primaryKeyColumn(qualified("id"), "INT") + ", " + qualified("name") + " VARCHAR(100), " + qualified("age") + " INT, " + qualified("email") + " VARCHAR(100), " + qualified("create_time") + " " + profile().datetimeColumnType() + ")");
