...
public final class Elastic7KeyMapperFixture implements AutoCloseable {
    ...
    public void prepareReservation(String name) throws SQLException {
        new JdbcTemplate(fixture.connection()).executeUpdate("POST /" + fixture.index() + "/_doc {\"reserved_for\": ?,\"reserved_key\": ?}", new Object[] { name, java.util.UUID.randomUUID().toString() });
    }

    public Session session() throws Exception {
        Session session = fixture.session();
        String path = "/" + fixture.index();
        session.getConfiguration().addMacro("esKeyInsert", "POST " + path + "/_doc {\"name\": #{name},\"age\": #{age},\"email\": #{email}}");
        session.getConfiguration().addMacro("esKeyExplicit", "PUT " + path + "/_doc/${id} {\"name\": #{name},\"age\": #{age},\"email\": #{email}}");
        session.getConfiguration().addMacro("esKeyRead", "POST " + path + "/_search {\"_source\": [\"name\"],\"query\": {\"ids\": {\"values\": [#{id}]}}}");
        session.getConfiguration().addMacro("esKeyBefore", "POST " + path + "/_search {\"_source\": [\"reserved_key\"],\"query\": {\"term\": {\"reserved_for.keyword\": #{name}}}}");
        session.getConfiguration().addMacro("esKeyAfterInsert", "PUT " + path + "/_doc/${name} {\"name\": #{name},\"age\": #{age},\"email\": #{email},\"key_ref\": #{name}}");
        session.getConfiguration().addMacro("esKeyAfter", "POST " + path + "/_search {\"_source\": [\"key_ref\"],\"query\": {\"term\": {\"name\": #{name}}}}");
        return session;
    }
    ...
    public interface NativeKeys {
        ...
        @SelectKeySql(value = "@{macro, esKeyBefore}", keyProperty = "id", order = Order.Before, statementType = StatementType.Prepared, timeout = 30, fetchSize = 1, resultSetType = ResultSetType.DEFAULT)
        @Insert("@{macro, esKeyExplicit}")
