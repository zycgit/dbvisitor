...
public interface MySqlSelectKeyMapper extends AnnotationAttributesMapper {
    ...
    @Override
    @SelectKeySql(value = "SELECT -959102", keyProperty = "id", order = Order.Before, statementType = StatementType.Prepared, timeout = 30, fetchSize = 1, resultSetType = ResultSetType.DEFAULT)
    @Insert("""
            INSERT INTO user_info (id, name, age, email, create_time)
            VALUES (#{id}, #{name}, #{age}, #{email}, #{createTime})
            """)
