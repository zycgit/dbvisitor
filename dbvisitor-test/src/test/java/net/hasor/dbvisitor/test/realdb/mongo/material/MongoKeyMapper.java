...
public interface MongoKeyMapper {
    ...
    @SelectKeySql(value = "@{macro, mongoKeySource}.find({name: #{name}}, {_id: 1})", keyProperty = "id", order = Order.Before, statementType = StatementType.Prepared, timeout = 30, fetchSize = 1, resultSetType = ResultSetType.DEFAULT)
    @Insert("@{macro, mongoSource}.insert({_id: ObjectId(#{id}), name: #{name}})")
