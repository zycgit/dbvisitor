...
public abstract class LambdaJdbcReadbackCase extends LambdaCrudSupport {
    // 能力归属：构造器 API / 写入操作。
    @Test
    @Capability(value = CapabilityId.LAMBDA_JDBC_INSERT_READBACK, column = "builder/inserts-updates-and-deletes/writes")
    public void lambdaInsert_shouldAllowJdbcStringReadback() throws SQLException {
        int id = baseId() + 1;
        lambdaTemplate.insert(UserInfo.class).applyEntity(user(id, "NXN-Lambda-Entity-Insert", 41, "nxn-lambda-entity-insert@test.com")).executeSumResult();

        assertEquals("NXN-Lambda-Entity-Insert", jdbcTemplate.queryForString(command(JdbcCrudCommand.SELECT_NAME), new Object[] { id }));
    }

    // 能力归属：构造器 API / 写入操作。
    @Test
    @Capability(value = CapabilityId.LAMBDA_JDBC_UPDATE_READBACK, column = "builder/inserts-updates-and-deletes/writes")
    public void lambdaUpdate_shouldAllowJdbcIntegerReadback() throws SQLException {
        ...
        assertEquals(Integer.valueOf(44), jdbcTemplate.queryForObject(command(JdbcCrudCommand.SELECT_AGE), new Object[] { id }, Integer.class));
    }

    // 能力归属：构造器 API / 写入操作。
    @Test
    @Capability(value = CapabilityId.LAMBDA_JDBC_DELETE_READBACK, column = "builder/inserts-updates-and-deletes/writes")
