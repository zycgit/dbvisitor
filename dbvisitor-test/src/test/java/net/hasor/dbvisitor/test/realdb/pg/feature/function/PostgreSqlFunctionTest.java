...
public class PostgreSqlFunctionTest extends FunctionCase {
    @Override
    ...
    protected void createFunctionDefinitions() throws SQLException {
        ...
        jdbcTemplate.execute("""
            CREATE OR REPLACE FUNCTION nxn_fn_get_username(user_id INT)
            ...
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE FUNCTION nxn_fn_transform_string(INOUT text_value VARCHAR, IN suffix VARCHAR)
            AS $$ BEGIN text_value := UPPER(text_value) || suffix; END; $$ LANGUAGE plpgsql
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE FUNCTION nxn_fn_calc_numbers(
            ...
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE FUNCTION nxn_fn_multi_resultsets()
            ...
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE FUNCTION nxn_fn_filter_users(IN min_age INT)
            ...
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE FUNCTION nxn_fn_complex_params(IN input_id INT, INOUT counter INT, OUT user_name VARCHAR, OUT user_age INT)
