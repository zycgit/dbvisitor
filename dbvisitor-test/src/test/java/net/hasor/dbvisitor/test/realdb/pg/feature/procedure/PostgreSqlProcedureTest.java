...
public class PostgreSqlProcedureTest extends ProcedureCase {
    @Override
    ...
    protected void createProcedureDefinitions() throws SQLException {
        ...
        jdbcTemplate.execute("""
            ...
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE PROCEDURE nxn_sp_transform_string(INOUT text_value VARCHAR, IN suffix VARCHAR)
            LANGUAGE plpgsql AS $$ BEGIN text_value := UPPER(text_value) || suffix; END $$
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE PROCEDURE nxn_sp_get_user_info(IN user_id INT, INOUT user_name VARCHAR, INOUT user_age INT)
            LANGUAGE plpgsql AS $$ BEGIN SELECT name, age INTO user_name, user_age FROM user_info WHERE id = user_id; END $$
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE PROCEDURE nxn_sp_update_counter(INOUT counter INT, IN increment INT)
            LANGUAGE plpgsql AS $$ BEGIN counter := counter + increment; END $$
            """);
        jdbcTemplate.execute("""
            CREATE OR REPLACE PROCEDURE nxn_sp_cursor_users(IN p_name VARCHAR, INOUT p_cursor refcursor)
