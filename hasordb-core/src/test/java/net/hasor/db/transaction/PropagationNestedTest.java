...
public class PropagationNestedTest extends AbstractPropagationTest {
    ...
    public void tran_rollback_test_1() throws Throwable {
        try (DruidDataSource dataSource = DsUtils.mysqlDataSource();//
            ...
            /* T1 */
            TransactionStatus tran1 = tranManager.getTransaction(Propagation.NESTED, Isolation.REPEATABLE_READ);
            jdbcTemplate.executeUpdate(INSERT_ARRAY, arrayForData1());
            assert selectCount(dataSource) == 1;
            assert selectCount(conn) == 0;

            /* T2 */
            TransactionStatus tran2 = tranManager.getTransaction(Propagation.NESTED, Isolation.REPEATABLE_READ);
