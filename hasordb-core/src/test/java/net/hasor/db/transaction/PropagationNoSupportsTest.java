...
public class PropagationNoSupportsTest extends AbstractPropagationTest {
    ...
    public void tran_rollback_test_1() throws Throwable {
        try (DruidDataSource dataSource = DsUtils.mysqlDataSource();//
            ...
            /* T2 */
            TransactionStatus tran2 = tranManager.getTransaction(Propagation.NOT_SUPPORTED, Isolation.REPEATABLE_READ);
    ...
    public void tran_commit_test_1() throws Throwable {
        try (DruidDataSource dataSource = DsUtils.mysqlDataSource();//
            ...
            /* T1 */
            TransactionStatus tran1 = tranManager.getTransaction(Propagation.REQUIRED, Isolation.READ_COMMITTED);
            jdbcTemplate.executeUpdate(INSERT_ARRAY, arrayForData1());
            assert selectCount(dataSource) == 1;
            assert selectCount(conn) == 0;

            /* T2 */
            TransactionStatus tran2 = tranManager.getTransaction(Propagation.NOT_SUPPORTED, Isolation.READ_COMMITTED);
            jdbcTemplate.executeUpdate(INSERT_ARRAY, arrayForData2());
            jdbcTemplate.executeUpdate(INSERT_ARRAY, arrayForData3());
            assert selectCount(dataSource) == 2;
            assert selectCount(conn) == 2;
