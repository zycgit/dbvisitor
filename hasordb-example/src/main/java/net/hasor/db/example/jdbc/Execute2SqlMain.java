...
public class Execute2SqlMain {
    public static void main(String[] args) throws SQLException, IOException {
        ...
        int queryForInt = jdbcTemplate.queryForInt("select count(*) from test_user where age > ?", 20);
        System.out.println(queryForInt);

        long queryForLong = jdbcTemplate.queryForLong("select count(*) from test_user where age > ?", 20);
        ...
        int result1 = jdbcTemplate.executeUpdate("update test_user set name = ? where id = ?", "mala", 1);
        System.out.println(result1);

        int result2 = jdbcTemplate.executeUpdate("delete from test_user where id = ?", 1);
        System.out.println(result2);

        int result3 = jdbcTemplate.executeUpdate("insert into `test_user` values (?,?,?,?)", 10, "'david'", 26, new Date());
